...
main (int argc, char *argv[])
{
  ...
  std::string vmInput ("input/virt-5gc-vminput-low.input");
  ...
  for (uint16_t i = 0; i < 3; i++) {
	positionAlloc->Add(Vector(10 * i, 5*i, 0));
  }
...
*/
	  ...
	  Ipv4Address ueipaddress = ueIpIface.GetAddress(0);
	  ...
	  ApplicationContainer sourceApps = source.Install (remoteHost);
	  sourceApps.Start (Seconds (start + 0.2));
	  sourceApps.Stop (Seconds (simTime));
	  ...
	  EpsBearer bearer (EpsBearer::NGBR_VIDEO_TCP_OPERATOR);
	  nrHelper->ActivateDedicatedEpsBearer (ueNrDevs.Get(0), bearer, tft);
...
*/
  ...
  //Simulator::Schedule (Seconds (1 + numberOfNodes*0.2), &CalcThroughput, sinkApps, 0, 0);
