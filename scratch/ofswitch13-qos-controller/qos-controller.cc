...
NS_LOG_COMPONENT_DEFINE ("QosController1");
NS_OBJECT_ENSURE_REGISTERED (QosController1);

QosController1::QosController1 ()
{
...
TypeId
QosController1::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::QosController1")
    .SetParent<OFSwitch13Controller> ()
    .SetGroupName ("OFSwitch13")
    .AddConstructor<QosController1> ()
    .AddAttribute ("EnableMeter",
                   "Enable per-flow mettering.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&QosController1::m_meterEnable),
                   MakeBooleanChecker ())
    .AddAttribute ("MeterRate",
                   "Per-flow meter rate.",
                   DataRateValue (DataRate ("256Kbps")),
                   MakeDataRateAccessor (&QosController1::m_meterRate),
                   MakeDataRateChecker ())
    .AddAttribute ("LinkAggregation",
                   "Enable link aggregation.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&QosController1::m_linkAggregation),
                   MakeBooleanChecker ())
    .AddAttribute ("ServerIpAddr",
                   "Server IPv4 address.",
                   AddressValue (Address (Ipv4Address ("10.1.1.1"))),
                   MakeAddressAccessor (&QosController1::m_serverIpAddress),
                   MakeAddressChecker ())
    .AddAttribute ("ServerTcpPort",
                   "Server TCP port.",
                   UintegerValue (9),
                   MakeUintegerAccessor (&QosController1::m_serverTcpPort),
                   MakeUintegerChecker<uint64_t> ())
    .AddAttribute ("ServerMacAddr",
                   "Server MAC address.",
                   AddressValue (Address (Mac48Address ("00:00:00:00:00:01"))),
                   MakeAddressAccessor (&QosController1::m_serverMacAddress),
                   MakeAddressChecker ())
  ;
  return tid;
}
...
void
QosController1::HandshakeSuccessful (Ptr<const RemoteSwitch> swtch)
{
...
void
QosController1::ConfigureBorderSwitch (Ptr<const RemoteSwitch> swtch)
{
  NS_LOG_FUNCTION (this << swtch);
...
void
QosController1::ConfigureAggregationSwitch (Ptr<const RemoteSwitch> swtch)
{
  NS_LOG_FUNCTION (this << swtch);
...
ofl_err
QosController1::HandleArpPacketIn (
  struct ofl_msg_packet_in *msg, Ptr<const RemoteSwitch> swtch,
  uint32_t xid)
{
  NS_LOG_FUNCTION (this << swtch << xid);
...
ofl_err
QosController1::HandleConnectionRequest (
  struct ofl_msg_packet_in *msg, Ptr<const RemoteSwitch> swtch,
  uint32_t xid)
{
  NS_LOG_FUNCTION (this << swtch << xid);
...
Ipv4Address
QosController1::ExtractIpv4Address (uint32_t oxm_of, struct ofl_match* match)
{
  switch (oxm_of)
    {
    case OXM_OF_ARP_SPA:
...
Ptr<Packet>
QosController1::CreateArpRequest (Mac48Address srcMac, Ipv4Address srcIp,
                                 Ipv4Address dstIp)
{
...
Ptr<Packet>
QosController1::CreateArpReply (Mac48Address srcMac, Ipv4Address srcIp,
...
void
QosController1::SaveArpEntry (Ipv4Address ipAddr, Mac48Address macAddr)
{
...
Mac48Address
QosController1::GetArpEntry (Ipv4Address ip)
{
