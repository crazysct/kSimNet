...
NS_LOG_COMPONENT_DEFINE ("LteHelper1");

NS_OBJECT_ENSURE_REGISTERED (LteHelper1);

LteHelper1::LteHelper1 (void)
...
TypeId LteHelper1::GetTypeId (void)
{
  static TypeId
    tid =
    TypeId ("ns3::LteHelper1")
    .SetParent<Object> ()
    .AddConstructor<LteHelper1> ()
    .AddAttribute ("Scheduler",
                   "The type of scheduler to be used for eNBs. "
                   "The allowed values for this attributes are the type names "
                   "of any class inheriting from ns3::FfMacScheduler.",
                   StringValue ("ns3::PfFfMacScheduler"),
                   MakeStringAccessor (&LteHelper1::SetSchedulerType,
                                       &LteHelper1::GetSchedulerType),
                   MakeStringChecker ())
    .AddAttribute ("FfrAlgorithm",
                   ...
                   MakeStringAccessor (&LteHelper1::SetFfrAlgorithmType,
                                       &LteHelper1::GetFfrAlgorithmType),
                   MakeStringChecker ())
    .AddAttribute ("HandoverAlgorithm",
                   ...
                   MakeStringAccessor (&LteHelper1::SetHandoverAlgorithmType,
                                       &LteHelper1::GetHandoverAlgorithmType),
                   MakeStringChecker ())
    .AddAttribute ("PathlossModel",
                   ...
                   MakeStringAccessor (&LteHelper1::SetPathlossModelType),
                   MakeStringChecker ())
    .AddAttribute ("FadingModel",
                   ...
                   StringValue (""),
                   MakeStringAccessor (&LteHelper1::SetFadingModel),
                   MakeStringChecker ())
    .AddAttribute ("UseIdealRrc",
                   ...
                   BooleanValue (true), 
                   MakeBooleanAccessor (&LteHelper1::m_useIdealRrc),
                   MakeBooleanChecker ())
    .AddAttribute ("AnrEnabled",
                   "Activate or deactivate Automatic Neighbour Relation function",
                   BooleanValue (true),
                   MakeBooleanAccessor (&LteHelper1::m_isAnrEnabled),
                   MakeBooleanChecker ())
    .AddAttribute ("UsePdschForCqiGeneration",
                   ...
                   BooleanValue (true),
                   MakeBooleanAccessor (&LteHelper1::m_usePdschForCqiGeneration),
                   MakeBooleanChecker ())
  ;
  return tid;
}
...
void 
LteHelper1::SetSchedulerType (std::string type) 
{
...
std::string
LteHelper1::GetSchedulerType () const
{
  return m_schedulerFactory.GetTypeId ().GetName ();
} 

void 
LteHelper1::SetSchedulerAttribute (std::string n, const AttributeValue &v)
{
...
std::string
LteHelper1::GetFfrAlgorithmType () const
{
  return m_ffrAlgorithmFactory.GetTypeId ().GetName ();
}

void
LteHelper1::SetFfrAlgorithmType (std::string type)
{
  NS_LOG_FUNCTION (this << type);
...
void
LteHelper1::SetFfrAlgorithmAttribute (std::string n, const AttributeValue &v)
{
  NS_LOG_FUNCTION (this << n);
...
std::string
LteHelper1::GetHandoverAlgorithmType () const
{
  return m_handoverAlgorithmFactory.GetTypeId ().GetName ();
}

void
LteHelper1::SetHandoverAlgorithmType (std::string type)
{
  NS_LOG_FUNCTION (this << type);
...
void
LteHelper1::SetHandoverAlgorithmAttribute (std::string n, const AttributeValue &v)
{
  NS_LOG_FUNCTION (this << n);
...
void 
LteHelper1::SetPathlossModelType (std::string type) 
{
  NS_LOG_FUNCTION (this << type);
...
void 
LteHelper1::SetPathlossModelAttribute (std::string n, const AttributeValue &v)
{
  NS_LOG_FUNCTION (this << n);
...
void
LteHelper1::SetEnbDeviceAttribute (std::string n, const AttributeValue &v)
{
  NS_LOG_FUNCTION (this);
...
void 
LteHelper1::SetEnbAntennaModelType (std::string type)
{
  NS_LOG_FUNCTION (this);
...
void 
LteHelper1::SetEnbAntennaModelAttribute (std::string n, const AttributeValue &v)
{
  NS_LOG_FUNCTION (this);
...
void
LteHelper1::SetUeDeviceAttribute (std::string n, const AttributeValue &v)
{
  NS_LOG_FUNCTION (this);
...
void 
LteHelper1::SetUeAntennaModelType (std::string type)
{
  NS_LOG_FUNCTION (this);
...
void 
LteHelper1::SetUeAntennaModelAttribute (std::string n, const AttributeValue &v)
{
  NS_LOG_FUNCTION (this);
...
void 
LteHelper1::SetFadingModel (std::string type) 
{
  NS_LOG_FUNCTION (this << type);
...
void 
LteHelper1::SetFadingModelAttribute (std::string n, const AttributeValue &v)
{
  m_fadingModelFactory.Set (n, v);
}

void 
LteHelper1::SetSpectrumChannelType (std::string type) 
{
  NS_LOG_FUNCTION (this << type);
...
void 
LteHelper1::SetSpectrumChannelAttribute (std::string n, const AttributeValue &v)
{
...
NetDeviceContainer1
LteHelper1::InstallEnbDevice (NodeContainer c)
{
  NS_LOG_FUNCTION (this);
...
NetDeviceContainer1
LteHelper1::InstallUeDevice (NodeContainer c)
{
  NS_LOG_FUNCTION (this);
  NetDeviceContainer1 devices;
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
...
Ptr<NetDevice1>
LteHelper1::InstallSingleEnbDevice (Ptr<Node> n)
{
  ...
  NS_ASSERT_MSG (mm, "MobilityModel needs to be set on node before calling LteHelper1::InstallUeDevice ()");
...
Ptr<NetDevice1>
LteHelper1::InstallSingleUeDevice (Ptr<Node> n)
{
  NS_LOG_FUNCTION (this);
  Ptr<LteSpectrumPhy> dlPhy = CreateObject<LteSpectrumPhy> ();
  Ptr<LteSpectrumPhy> ulPhy = CreateObject<LteSpectrumPhy> ();
...
void
LteHelper1::Attach (NetDeviceContainer1 ueDevices)
{
  NS_LOG_FUNCTION (this);
...
void
LteHelper1::Attach (Ptr<NetDevice1> ueDevice)
{
  NS_LOG_FUNCTION (this);
  ...
  if (ueLteDevice == 0)
    {
      NS_FATAL_ERROR ("The passed NetDevice1 must be an LteUeNetDevice");
    }
...
void
LteHelper1::Attach (NetDeviceContainer1 ueDevices, Ptr<NetDevice1> enbDevice)
{
  NS_LOG_FUNCTION (this);
  for (NetDeviceContainer1::Iterator i = ueDevices.Begin (); i != ueDevices.End (); ++i)
    {
...
void
LteHelper1::Attach (Ptr<NetDevice1> ueDevice, Ptr<NetDevice1> enbDevice)
{
  NS_LOG_FUNCTION (this);
...
void
LteHelper1::AttachToClosestEnb (NetDeviceContainer1 ueDevices, NetDeviceContainer1 enbDevices)
{
  NS_LOG_FUNCTION (this);
  for (NetDeviceContainer1::Iterator i = ueDevices.Begin (); i != ueDevices.End (); ++i)
    {
...
void
LteHelper1::AttachToClosestEnb (Ptr<NetDevice1> ueDevice, NetDeviceContainer1 enbDevices)
{
  NS_LOG_FUNCTION (this);
...
void
LteHelper1::AddX2Interface (NodeContainer enbNodes)
{
  NS_LOG_FUNCTION (this);
...
void
LteHelper1::AddX2Interface (Ptr<Node> enbNode1, Ptr<Node> enbNode2)
{
  NS_LOG_FUNCTION (this);
...
void
LteHelper1::DoHandoverRequest (Ptr<NetDevice1> ueDev, Ptr<NetDevice1> sourceEnbDev, Ptr<NetDevice1> targetEnbDev)
{
  NS_LOG_FUNCTION (this << ueDev << sourceEnbDev << targetEnbDev);
...
void
LteHelper1::DoDeActivateDedicatedEpsBearer (Ptr<NetDevice1> ueDevice, Ptr<NetDevice1> enbDevice, uint8_t bearerId)
{
  NS_LOG_FUNCTION (this << ueDevice << bearerId);
...
void 
LteHelper1::ActivateDataRadioBearer (NetDeviceContainer1 ueDevices, EpsBearer bearer)
{
  NS_LOG_FUNCTION (this);
  for (NetDeviceContainer1::Iterator i = ueDevices.Begin (); i != ueDevices.End (); ++i)
    {
...
void
LteHelper1::EnableLogComponents (void)
{
  LogComponentEnable ("LteHelper1", LOG_LEVEL_ALL);
...
void
LteHelper1::EnableRlcTraces (void)
{
  NS_ASSERT_MSG (m_rlcStats == 0, "please make sure that LteHelper1::EnableRlcTraces is called at most once");
  m_rlcStats = CreateObject<RadioBearerStatsCalculator> ("RLC");
  m_radioBearerStatsConnector.EnableRlcStats (m_rlcStats);
}

int64_t
LteHelper1::AssignStreams (NetDeviceContainer1 c, int64_t stream)
{
...
void
LteHelper1::EnablePhyTraces (void)
{
  EnableDlPhyTraces ();
  EnableUlPhyTraces ();
  EnableDlTxPhyTraces ();
...
void
LteHelper1::EnableDlTxPhyTraces (void)
{
...
void
LteHelper1::EnableUlTxPhyTraces (void)
{
...
void
LteHelper1::EnableDlRxPhyTraces (void)
{
...
void
LteHelper1::EnableUlRxPhyTraces (void)
{
...
void
LteHelper1::EnableMacTraces (void)
{
  EnableDlMacTraces ();
  EnableUlMacTraces ();
}
...
void
LteHelper1::EnableDlMacTraces (void)
{
...
void
LteHelper1::EnableUlMacTraces (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
LteHelper1::EnableDlPhyTraces (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
LteHelper1::EnableUlPhyTraces (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
LteHelper1::GetRlcStats (void)
{
  return m_rlcStats;
}

void
LteHelper1::EnablePdcpTraces (void)
{
  NS_ASSERT_MSG (m_pdcpStats == 0, "please make sure that LteHelper1::EnablePdcpTraces is called at most once");
  m_pdcpStats = CreateObject<RadioBearerStatsCalculator> ("PDCP");
  m_radioBearerStatsConnector.EnablePdcpStats (m_pdcpStats);
}

Ptr<RadioBearerStatsCalculator>
LteHelper1::GetPdcpStats (void)
{
  return m_pdcpStats;
}
