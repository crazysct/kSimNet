...
{
	std::cout << "SetS1apSapMme for m_s1apSapEnbProvider is called" << std::endl;
...
{
  std::cout << "SetS1apSapMme for m_s1apSapMme is called" << std::endl;
  m_s1apSapMme = s;
}
...
EpcEnbApplication::DoInitialUeMessage (uint64_t imsi, uint16_t rnti)
{
  ...
  //m_s1apSapMme->InitialUeMessage (imsi, rnti, imsi, m_cellId); // jhlim
  if(m_s1apSapEnbProvider == NULL)
...
/* jhlim */
void
EpcEnbApplication::DoInitialUeMessage (uint64_t imsi, uint16_t rnti, int dummy)
{
	NS_LOG_FUNCTION (this);
	m_imsiRntiMap[imsi] = rnti;
	m_s1apSapMme->InitialUeMessage (imsi, rnti, imsi, m_cellId);
...
/*
void 
EpcEnbApplication::DoPathSwitchRequest (EpcEnbS1SapProvider::PathSwitchRequestParameters params, int dummy)
{
  NS_LOG_FUNCTION (this);
  uint16_t enbUeS1Id = params.rnti;  
  uint64_t mmeUeS1Id = params.mmeUeS1Id;
...
/*
void
EpcEnbApplication::DoReleaseIndication (uint64_t imsi, uint16_t rnti, uint8_t bearerId, int dummy)
{
  NS_LOG_FUNCTION (this << bearerId );
  std::list<EpcS1apSapMme::ErabToBeReleasedIndication> erabToBeReleaseIndication;
  EpcS1apSapMme::ErabToBeReleasedIndication erab;
