...
#ifndef ORIGINAL_CSMA_CHANNEL_H
#define ORIGINAL_CSMA_CHANNEL_H
 ...
 */
class CsmaDeviceRec1 {
public:
  Ptr< CsmaNetDevice1 > devicePtr; //!< Pointer to the net device
   ...
   */
  CsmaDeviceRec1(Ptr< CsmaNetDevice1 > device);

  /**
   * Copy constructor
   * \param o the object to copy
   */
  CsmaDeviceRec1 (CsmaDeviceRec1 const &o);
...
enum WireState1
{
  IDLE1,            /**< Channel is IDLE, no packet is being transmitted */
  TRANSMITTING1,    /**< Channel is BUSY, a packet is being written by a net device */
  PROPAGATING1      /**< Channel is BUSY, packet is propagating to all attached net devices */
};
 ...
 */
class CsmaChannel1 : public Channel1 
{
public:
   ...
   */
  int32_t Attach (Ptr<CsmaNetDevice1> device);
   ...
   * can't be found.
   */
  bool Detach (Ptr<CsmaNetDevice1> device);
   ...
   * channel or can't be found.
   */
  bool Reattach (Ptr<CsmaNetDevice1> device);
   ...
   * number is needed
   */
  int32_t GetDeviceNum (Ptr<CsmaNetDevice1> device);
   ...
   * with deviceId i.
   */
  virtual Ptr<NetDevice1> GetDevice (uint32_t i) const;
   ...
   * with deviceId i.
   */
  Ptr<CsmaNetDevice1> GetCsmaDevice (uint32_t i) const;
...
private:
   ...
   */
  CsmaChannel1 (CsmaChannel1 const &o);
   ...
   */
  CsmaChannel1 &operator = (CsmaChannel1 const &o);
   ...
   */
  std::vector<CsmaDeviceRec1> m_deviceList;
