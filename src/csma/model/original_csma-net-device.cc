...
#include "original_csma-net-device.h"
#include "original_csma-channel.h"
...
CsmaNetDevice::TransmitStart (void)
{
  ...
  //
  if (m_channel->GetState () != IDLE) //sjkang_x
    {
      //
...
CsmaNetDevice::TransmitCompleteEvent (void)
{
  ...
  NS_ASSERT (m_channel->GetState () == TRANSMITTING); //sjkang_x
