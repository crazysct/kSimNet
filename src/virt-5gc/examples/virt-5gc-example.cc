...
{
	std::string nodeN = std::to_string(totalNodeN + 1);
...
CalcThroughput (Ptr<Application> sinkApp, int lastTotalRx, int node)
{
	...
	double throughput = (sink->GetTotalRx() - lastTotalRx) * (double)(8/1e5);
	...
	*thputStream->GetStream() << Simulator::Now().GetSeconds() << ", " << node << ", " << throughput << std::endl;

	Simulator::Schedule(MilliSeconds(100), &CalcThroughput, sinkApp, lastTotalRx, node);
}
...
main (int argc, char *argv[])
{
  ...
  std::string topoInput ("scratch/virt-5gc-toposample3.input");
  std::string vmInput ("scratch/virt-5gc-vminput.input");
  ...
  // set scaling delay
  virt5gcHelper->SetAllocationDelay(0.2);
