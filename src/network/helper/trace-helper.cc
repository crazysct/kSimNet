...
{
  NetDeviceContainer devs; //sjkang_x
  for (NodeContainer::Iterator i = n.Begin (); i != n.End (); ++i)
    {
      Ptr<Node> node = *i;
      for (uint32_t j = 0; j < node->GetNDevices (); ++j)
        {
          devs.Add (node->GetDevice (j)); //sjkang_x
        }
    }
...
/* jhlim
void 
PcapHelperForDevice1::EnablePcap (std::string prefix, Ptr<NetDevice1> nd, bool promiscuous, bool explicitFilename)
{
  EnablePcapInternal (prefix, nd, promiscuous, explicitFilename);
}

void 
PcapHelperForDevice1::EnablePcap (std::string prefix, std::string ndName, bool promiscuous, bool explicitFilename)
{
  Ptr<NetDevice1> nd = Names::Find<NetDevice1> (ndName);
  EnablePcap (prefix, nd, promiscuous, explicitFilename);
}

void 
PcapHelperForDevice1::EnablePcap (std::string prefix, NetDeviceContainer1 d, bool promiscuous)
{
  for (NetDeviceContainer1::Iterator i = d.Begin (); i != d.End (); ++i)
    {
      Ptr<NetDevice1> dev = *i;
      EnablePcap (prefix, dev, promiscuous);
    }
}

void
PcapHelperForDevice1::EnablePcap (std::string prefix, NodeContainer n, bool promiscuous)
{
  ...
  for (NodeContainer::Iterator i = n.Begin (); i != n.End (); ++i)
    {
      ...
      for (uint32_t j = 0; j < node->GetNDevices (); ++j)
        {
          devs.Add (node->GetDevice1 (j)); //sjkang_x
        }
    }
  EnablePcap (prefix, devs, promiscuous);
}
...
void
PcapHelperForDevice1::EnablePcapAll (std::string prefix, bool promiscuous)
{
  EnablePcap (prefix, NodeContainer::GetGlobal (), promiscuous);
}

void 
PcapHelperForDevice1::EnablePcap (std::string prefix, uint32_t nodeid, uint32_t deviceid, bool promiscuous)
{
  NodeContainer n = NodeContainer::GetGlobal ();
  ...
  for (NodeContainer::Iterator i = n.Begin (); i != n.End (); ++i)
    {
      ...
      NS_ABORT_MSG_IF (deviceid >= node->GetNDevices (), "PcapHelperForDevice::EnablePcap(): Unknown deviceid = " 
                       << deviceid);
      Ptr<NetDevice1> nd = node->GetDevice1 (deviceid); //sjkang_x
      EnablePcap (prefix, nd, promiscuous);
      return;
    }
}
...
/////////////////////////////////sjkang_x for changing PcapHelperForDevice class 
//
// Public API
//
/*
...
/* jhlim
void 
AsciiTraceHelperForDevice1::EnableAscii (std::string prefix, Ptr<NetDevice1> nd, bool explicitFilename)
{
  EnableAsciiInternal (Ptr<OutputStreamWrapper> (), prefix, nd, explicitFilename);
}
...
//
void 
AsciiTraceHelperForDevice1::EnableAscii (Ptr<OutputStreamWrapper> stream, Ptr<NetDevice1> nd)
{
  EnableAsciiInternal (stream, std::string (), nd, false);
}
...
//
void 
AsciiTraceHelperForDevice1::EnableAscii (std::string prefix, std::string ndName, bool explicitFilename)
{
  EnableAsciiImpl (Ptr<OutputStreamWrapper> (), prefix, ndName, explicitFilename);
}
...
//
void 
AsciiTraceHelperForDevice1::EnableAscii (Ptr<OutputStreamWrapper> stream, std::string ndName)
{
  EnableAsciiImpl (stream, std::string (), ndName, false);
}
...
//
void 
AsciiTraceHelperForDevice1::EnableAsciiImpl (
  Ptr<OutputStreamWrapper> stream, 
  std::string prefix, 
  std::string ndName,
  bool explicitFilename)
...
//
void 
AsciiTraceHelperForDevice1::EnableAscii (std::string prefix, NetDeviceContainer1 d)
{
  EnableAsciiImpl (Ptr<OutputStreamWrapper> (), prefix, d);
}
...
//
void 
AsciiTraceHelperForDevice1::EnableAscii (Ptr<OutputStreamWrapper> stream, NetDeviceContainer1 d)
{
  EnableAsciiImpl (stream, std::string (), d);
}
...
//
void 
AsciiTraceHelperForDevice1::EnableAsciiImpl (Ptr<OutputStreamWrapper> stream, std::string prefix, NetDeviceContainer1 d)
{
  for (NetDeviceContainer1::Iterator i = d.Begin (); i != d.End (); ++i)
    {
      Ptr<NetDevice1> dev = *i;
...
//
void
AsciiTraceHelperForDevice1::EnableAscii (std::string prefix, NodeContainer n)
{
  EnableAsciiImpl (Ptr<OutputStreamWrapper> (), prefix, n);
}
...
//
void
AsciiTraceHelperForDevice1::EnableAscii (Ptr<OutputStreamWrapper> stream, NodeContainer n)
{
  EnableAsciiImpl (stream, std::string (), n);
}
...
//
void
AsciiTraceHelperForDevice1::EnableAsciiImpl (Ptr<OutputStreamWrapper> stream, std::string prefix, NodeContainer n)
{
  NetDeviceContainer1 devs;
  for (NodeContainer::Iterator i = n.Begin (); i != n.End (); ++i)
    {
...
//
void
AsciiTraceHelperForDevice1::EnableAsciiAll (std::string prefix)
{
  EnableAsciiImpl (Ptr<OutputStreamWrapper> (), prefix, NodeContainer::GetGlobal ());
}
...
//
void
AsciiTraceHelperForDevice1::EnableAsciiAll (Ptr<OutputStreamWrapper> stream)
{
  EnableAsciiImpl (stream, std::string (), NodeContainer::GetGlobal ());
}
...
//
void 
AsciiTraceHelperForDevice1::EnableAscii (Ptr<OutputStreamWrapper> stream, uint32_t nodeid, uint32_t deviceid)
{
  EnableAsciiImpl (stream, std::string (), nodeid, deviceid, false);
}
...
//
void 
AsciiTraceHelperForDevice1::EnableAscii (
  std::string prefix, 
  uint32_t nodeid, 
  uint32_t deviceid,
  bool explicitFilename)
  ...
  for (NodeContainer::Iterator i = n.Begin (); i != n.End (); ++i)
    {
      ...
      Ptr<NetDevice1> nd = node->GetDevice1 (deviceid);
