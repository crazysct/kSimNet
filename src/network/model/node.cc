...
/* jhlim
uint32_t
Node::AddDevice1 (Ptr<NetDevice1> device)
{
  NS_LOG_FUNCTION (this << device);
  uint32_t index = m_devices1.size ();
  m_devices1.push_back (device);
  device->SetNode (this);
  device->SetIfIndex (index);
  device->SetReceiveCallback (MakeCallback (&Node::NonPromiscReceiveFromDevice1, this));
  Simulator::ScheduleWithContext (GetId (), Seconds (0.0), 
                                  &NetDevice1::Initialize, device);
  NotifyDeviceAdded1 (device);
  return index;
}
*/
Ptr<NetDevice>
...
/* jhlim
Ptr<NetDevice1>
Node::GetDevice1 (uint32_t index) const
{
  NS_LOG_FUNCTION (this << index);
  NS_ASSERT_MSG (index < m_devices1.size (), "Device index " << index <<
                 " is out of range (only have " << m_devices1.size () << " devices).");
  return m_devices1[index];
}
*/
uint32_t 
...
/* jhlim
void 
Node::NotifyDeviceAdded1 (Ptr<NetDevice1> device)
{
  NS_LOG_FUNCTION (this << device);
  for (DeviceAdditionListenerList1::iterator i = m_deviceAdditionListeners1.begin ();
       i != m_deviceAdditionListeners1.end (); i++)
    {
      (*i) (device);
    }  
}
