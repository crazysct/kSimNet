...
#include "original_traffic-control-layer.h"
...
NS_LOG_COMPONENT_DEFINE ("TrafficControlLayer1");

NS_OBJECT_ENSURE_REGISTERED (TrafficControlLayer1);

TypeId
TrafficControlLayer1::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TrafficControlLayer1")
    .SetParent<Object> ()
    .SetGroupName ("TrafficControl")
    .AddConstructor<TrafficControlLayer1> ()
    .AddAttribute ("RootQueueDiscList", "The list of root queue discs associated to this Traffic Control layer.",
                   ObjectMapValue (),
                   MakeObjectMapAccessor (&TrafficControlLayer1::GetNDevices,
                                          &TrafficControlLayer1::GetRootQueueDiscOnDeviceByIndex),
...
TypeId
TrafficControlLayer1::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

TrafficControlLayer1::TrafficControlLayer1 ()
  : Object ()
{
...
void
TrafficControlLayer1::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  m_node = 0;
  m_handlers.clear ();
...
TrafficControlLayer1::NetDeviceInfo::NetDeviceInfo (Ptr<QueueDisc> rootQueueDisc,
...
TrafficControlLayer1::NetDeviceInfo::~NetDeviceInfo ()
{
  NS_LOG_FUNCTION (this);
}

void
TrafficControlLayer1::DoInitialize (void)
{
  NS_LOG_FUNCTION (this);
  std::map<Ptr<NetDevice1>, NetDeviceInfo>::iterator ndi;
  for (ndi = m_netDevices.begin (); ndi != m_netDevices.end (); ndi++)
    {
      ...
      if (ndi->second.m_rootQueueDisc)
        {
          // set the wake callbacks on NetDevice1 queues
           if (ndi->second.m_rootQueueDisc->GetWakeMode () == QueueDisc::WAKE_ROOT)
          ...
          else if (ndi->second.m_rootQueueDisc->GetWakeMode () == QueueDisc::WAKE_CHILD)
            {
              NS_ASSERT_MSG (ndi->second.m_rootQueueDisc->GetNQueueDiscClasses () == devQueueIface->GetNTxQueues (),
                             "The number of child queue discs does not match the number of NetDevice1 queues");
              for (uint32_t i = 0; i < devQueueIface->GetNTxQueues (); i++)
                {
...
void
TrafficControlLayer1::SetupDevice (Ptr<NetDevice1> device)
{
  ...
  // create a NetDevice1QueueInterface object and aggregate it to the device
  ...
  // create an entry in the m_NetDevice1s map for this device
  NS_ASSERT_MSG (m_netDevices.find (device) == m_netDevices.end (), "This is a bug,"
                 << "  SetupDevice only can insert an entry in the m_NetDevice1s map");
...
void
TrafficControlLayer1::RegisterProtocolHandler (Node::ProtocolHandler1 handler,
                                              uint16_t protocolType, Ptr<NetDevice1> device)
{
  ...
  NS_LOG_DEBUG ("Handler for NetDevice1: " << device << " registered for protocol " <<
                protocolType << ".");
}

void
TrafficControlLayer1::SetRootQueueDiscOnDevice (Ptr<NetDevice1> device, Ptr<QueueDisc> qDisc)
{
  NS_LOG_FUNCTION (this << device << qDisc);

  std::map<Ptr<NetDevice1>, NetDeviceInfo>::iterator ndi = m_netDevices.find (device);
  ...
  if (ndi == m_netDevices.end ())
    {
      ...
      // Since queue discs require that a NetDevice1 queue interface is aggregated
...
Ptr<QueueDisc>
TrafficControlLayer1::GetRootQueueDiscOnDevice (Ptr<NetDevice1> device) const
{
  NS_LOG_FUNCTION (this << device);

  std::map<Ptr<NetDevice1>, NetDeviceInfo>::const_iterator ndi = m_netDevices.find (device);
...
Ptr<QueueDisc>
TrafficControlLayer1::GetRootQueueDiscOnDeviceByIndex (uint32_t index) const
{
...
void
TrafficControlLayer1::DeleteRootQueueDiscOnDevice (Ptr<NetDevice1> device)
{
  NS_LOG_FUNCTION (this << device);
...
void
TrafficControlLayer1::SetNode (Ptr<Node> node)
{
...
void
TrafficControlLayer1::NotifyNewAggregate ()
{
  NS_LOG_FUNCTION (this);
  if (m_node == 0)
    {
...
uint32_t
TrafficControlLayer1::GetNDevices (void) const
{
...
void
TrafficControlLayer1::Receive (Ptr<NetDevice1> device, Ptr<const Packet> p,
                              uint16_t protocol, const Address &from, const Address &to,
                              NetDevice1::PacketType packetType)
{
          ...
          if (i->protocol == 0
              || i->protocol == protocol)
            {
              NS_LOG_DEBUG ("Found handler for packet " << p << ", protocol " <<
                            protocol << " and NetDevice1 " << device <<
                            ". Send packet up");
...
void
TrafficControlLayer1::Send (Ptr<NetDevice1> device, Ptr<QueueDiscItem> item)
{
  ...
  if (ndi->second.m_rootQueueDisc == 0)
    ...
    {
      // Enqueue the packet in the queue disc associated with the NetDevice1 queue
