 ...
 * protocol number.
 */
class TrafficControlLayer1 : public Object
{
public:
  /**
   ...
   * \brief Constructor
   */
  TrafficControlLayer1 ();

  virtual ~TrafficControlLayer1 ();
   ...
   *        devices.
   */
  void RegisterProtocolHandler (Node::ProtocolHandler1 handler,
                                uint16_t protocolType,
                                Ptr<NetDevice1> device);
   ...
   */
  virtual void SetupDevice (Ptr<NetDevice1> device);
   ...
   */
  virtual void SetRootQueueDiscOnDevice (Ptr<NetDevice1> device, Ptr<QueueDisc> qDisc);
   ...
   */
  virtual Ptr<QueueDisc> GetRootQueueDiscOnDevice (Ptr<NetDevice1> device) const;
   ...
   */
  virtual void DeleteRootQueueDiscOnDevice (Ptr<NetDevice1> device);
   ...
   */
  virtual void Receive (Ptr<NetDevice1> device, Ptr<const Packet> p,
                        uint16_t protocol, const Address &from,
                        const Address &to, NetDevice1::PacketType packetType);
  /**
   ...
   */
  virtual void Send (Ptr<NetDevice1> device, Ptr<QueueDiscItem> item);
...
private:
   ...
   */
  TrafficControlLayer1 (TrafficControlLayer1 const &);
  /**
   * \brief Assignment operator
   * \return this object
   * Disable default implementation to avoid misuse
   */
  TrafficControlLayer1& operator= (TrafficControlLayer1 const &);
  /**
   * \brief Protocol handler entry.
   * This structure is used to demultiplex all the protocols.
   */
  struct ProtocolHandlerEntry {
    Node::ProtocolHandler1 handler; //!< the protocol handler
  ...
  std::map<Ptr<NetDevice1>, NetDeviceInfo> m_netDevices;
