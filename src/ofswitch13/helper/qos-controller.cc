...
{
  //cout<<"QoS Controller Begins"<<endl;
...
QosController::HandshakeSuccessful (Ptr<const RemoteSwitch> swtch)
{
  ...
  //cout<<"swtch->GetDpId():"<<swtch->GetDpId()<<", swtch->GetAddress(): "<<swtch->GetIpv4()<<endl;
  if (swtch->GetDpId () == 1)
    {
...
// input: SDN border switch
// output: void
// Explanation: This configures botrder switches. It can specify the weight and group the output ports. 
void
QosController::ConfigureBorderSwitch (Ptr<const RemoteSwitch> swtch)
{
  ...
  //cout<<"border, m_linkAggregation:"<<m_linkAggregation<<endl;

  ifstream openFile("ovs_weight.txt");
  ...
  if( openFile.is_open())
  {
    ...
    {
      tmp_weight[tmp_cnt] = string(line);
      tmp_cnt++;
    }
    openFile.close();
  }

  string firstCommand = "weight=" + tmp_weight[0] + ",port=any,group=any output=1";
  string secondCommand = "weight=" + tmp_weight[1] + ",port=any,group=any output=2";

  //cout<<"command list"<<endl;
  //cout <<firstCommand<<", "<<secondCommand<<endl;
  ...
  if (m_linkAggregation)
    {
      ...
      */
      /*
      DpctlExecute (swtch, "group-mod cmd=add,type=sel,group=3 "
                    "weight=10,port=any,group=any output=1 "
                    "weight=1,port=any,group=any output=2");
       */

      DpctlExecute (swtch, "group-mod cmd=add,type=sel,group=3 "
                    "weight="+tmp_weight[0]+",port=any,group=any output=1 "
                    "weight="+tmp_weight[1]+",port=any,group=any output=2");
  ...
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=900 "
                "in_port=2 write:output=3");

  // Packets from input port 3 are redirected to group 1
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
                "in_port=3 write:group=3");
...
QosController::ConfigureAggregationSwitch (Ptr<const RemoteSwitch> swtch)
{
  ...
  //cout<<"aggregation, m_linkAggregation:"<<m_linkAggregation<<endl;
  if (m_linkAggregation)
    {
