...
map<Address, unsigned long long int> stat_ovs;
...
OFSwitch13Port::~OFSwitch13Port ()
{
  /* jhlim */
  ofstream outfile;
  outfile.open("ovs_lte_result.txt", std::ios_base::app);

  map<Address, unsigned long long int>::iterator iter;
  for (iter = m_ovsstat.begin(); iter != m_ovsstat.end(); ++iter)
  {
	  //
	  //
  }
    ...
    m_openflowDev (openflowDev)
{
  ...
  /* jhlim */
  string filePath="ovs_lte_result.txt";
  ofstream writeFile(filePath.data());
  if(writeFile.is_open())
  {
	  writeFile << "<experiment begins>\n";
	  writeFile.close();
  }
                         ...
                         const Address &to, NetDevice::PacketType packetType)
{
  ...
  //cout<<"pkt received: "<<device->GetAddress() <<endl;
  ...
  ifstream ifile("ovs_lte_result.txt");
  if (ifile && first == false)
  {
	  remove("ovs_lte_result.txt");
	  first = true;
  }
  ...
  Mac48Address tmp_addr1 = Mac48Address("02-06-00:00:00:00:00:03");
  Mac48Address tmp_addr2 = Mac48Address("02-06-00:00:00:00:00:05");

  if(device->GetAddress() == tmp_addr1 || device->GetAddress() == tmp_addr2)
	  outfile << Simulator::Now().GetSeconds() << ", " << device->GetAddress() << ", " << m_swPort->stats->rx_bytes << endl;

  m_ovsstat[device->GetAddress()] = m_swPort->stats->rx_bytes;
