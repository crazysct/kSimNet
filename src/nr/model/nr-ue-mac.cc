...
public:
  ...
  virtual void AddLc (uint8_t lcId, NrUeCmacSapProvider::LogicalChannelConfig lcConfig, NrMacSapUser* msu);
...
class UeMemberNrUePhySapUser : public NrUePhySapUser
{
public:
  UeMemberNrUePhySapUser (NrUeMac* mac);

  // inherited from NrPhySapUser
...
UeMemberNrUePhySapUser::UeMemberNrUePhySapUser (NrUeMac* mac) : m_mac (mac)
{

}

void
UeMemberNrUePhySapUser::ReceivePhyPdu (Ptr<Packet> p)
{
...
void
UeMemberNrUePhySapUser::SubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  m_mac->DoSubframeIndication (frameNo, subframeNo);
}

void
UeMemberNrUePhySapUser::ReceiveNrControlMessage (Ptr<NrControlMessage> msg)
{
  ...
  
{
  ...
  m_uePhySapUser = new UeMemberNrUePhySapUser (this);
...
NrUePhySapUser*
NrUeMac::GetNrUePhySapUser (void)
{
  return m_uePhySapUser;
}

void
NrUeMac::SetNrUePhySapProvider (NrUePhySapProvider* s)
{
...
NrMacSapProvider*
NrUeMac::GetNrMacSapProvider (void)
{
  return m_macSapProvider;
}

void
NrUeMac::SetNrUeCmacSapUser (NrUeCmacSapUser* s)
{
  m_cmacSapUser = s;
}

NrUeCmacSapProvider*
NrUeMac::GetNrUeCmacSapProvider (void)
{
...
NrUeMac::DoTransmitPdu (NrMacSapProvider::TransmitPduParameters params)
{
  ...
  NrRadioBearerTag tag (params.rnti, params.lcid, 0 /* UE works in SISO mode*/);
...
{  
  ...
  if (it != m_ulBsrReceived.end ())
    ...
    {
      m_ulBsrReceived.insert (std::pair<uint8_t, NrMacSapProvider::ReportBufferStatusParameters> (params.lcid, params));
    }
  m_freshUlBsr = true;
}
...
NrUeMac::SendReportBufferStatus (void)
{
  ...
  for (it = m_ulBsrReceived.begin (); it != m_ulBsrReceived.end (); it++)
    {
      ...
      NS_LOG_INFO("NR lcid of BSR " << (uint16_t)lcid << " queue size " << (*it).second.txQueueSize);
  ...
  bsr.m_macCeValue.m_bufferStatus.push_back (NrBufferSizeLevelBsr::BufferSize2BsrId (queue.at (0)));
  bsr.m_macCeValue.m_bufferStatus.push_back (NrBufferSizeLevelBsr::BufferSize2BsrId (queue.at (1)));
  bsr.m_macCeValue.m_bufferStatus.push_back (NrBufferSizeLevelBsr::BufferSize2BsrId (queue.at (2)));
  bsr.m_macCeValue.m_bufferStatus.push_back (NrBufferSizeLevelBsr::BufferSize2BsrId (queue.at (3)));

  // create the feedback to eNB
  Ptr<BsrNrControlMessage> msg = Create<BsrNrControlMessage> ();
  msg->SetBsr (bsr);
  m_uePhySapProvider->SendNrControlMessage (msg);
...
NrUeMac::SendRaPreamble (bool contention)
{
  ...
  // Since regular UL NrControlMessages need m_ulConfigured = true in
  // order to be sent by the UE, the rach preamble needs to be sent
  // with a dedicated primitive (not
  // m_uePhySapProvider->SendNrControlMessage (msg)) so that it can
  ...
  NS_LOG_FUNCTION (this << " sent preamble id " << (uint32_t) m_raPreambleId << ", RA-RNTI " << (uint32_t) m_raRnti);
  // 3GPP 36.321 5.1.4 
...
NrUeMac::RecvRaResponse (NrBuildRarListElement_s raResponse)
{
  ...
  std::map <uint8_t, NrMacSapProvider::ReportBufferStatusParameters>::iterator lc0BsrIt
...
void
NrUeMac::DoAddLc (uint8_t lcId,  NrUeCmacSapProvider::LogicalChannelConfig lcConfig, NrMacSapUser* msu)
{
  ...
  if (tag.GetRnti () == m_rnti)
    {
      ...
      if (it != m_lcInfoMap.end ())
        {
          NS_LOG_INFO("NrUeMac ReceivePhyPdu");
...
void
NrUeMac::DoReceiveNrControlMessage (Ptr<NrControlMessage> msg)
{
  NS_LOG_FUNCTION (this);
  if (msg->GetMessageType () == NrControlMessage::UL_DCI)
      ...
      if (dci.m_ndi == 1)
        {
          ...
          std::map <uint8_t, NrMacSapProvider::ReportBufferStatusParameters>::iterator itBsr;
      ...
      if (m_waitingForRaResponse)
        {
          ...
          if (raRnti == m_raRnti) // RAR corresponds to TX subframe of preamble
            {
              for (std::list<RarNrControlMessage::Rar>::const_iterator it = rarMsg->RarListBegin ();
    ...
    {
      NS_LOG_WARN (this << " NrControlMessage not recognized");
    }
}
...
void
NrUeMac::DoSubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  NS_LOG_FUNCTION (this);
  m_frameNo = frameNo;
