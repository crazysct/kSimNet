    ...
    m_expectedSeqNumber (0)
{
  ...
  m_ngcX2RlcUser = new NgcX2RlcSpecificUser<NrRlcUm> (this);
...
{
  static TypeId tid = TypeId ("ns3::NrRlcUm")
    .SetParent<NrRlc> ()
    .SetGroupName("Nr")
...
void 
NrRlcUm::DoSendMcPdcpSdu(NgcX2Sap::UeDataParams params)
{
  NS_LOG_FUNCTION(this);
...
NrRlcUm::DoReceivePdu (Ptr<Packet> p)
{
 ...
 // {
	...
	//  this->CalculatePathThroughput(this->stream);
		m_Tput_Calculator = Simulator::Schedule (MilliSeconds (100), &NrRlcUm::CalculatePathThroughput,this , stream);
	}
      ...
      if ( m_vrUh > m_vrUr )
        {
          ...
          m_reorderingTimer = Simulator::Schedule (Time ("0.1s"),
                                                   &NrRlcUm::ExpireReorderingTimer ,this);
          m_vrUx = m_vrUh;
...
bool
NrRlcUm::IsInsideReorderingWindow (SequenceNumber10 seqNumber)
{
...
void
NrRlcUm::TriggerReceivePdcpPdu(Ptr<Packet> p)
{
  if(!isMc) 
  {
    NS_LOG_INFO(this << " RlcUm forwards packet to PDCP (either from MmWave or NR stack)");
...
void
NrRlcUm::DoSendAssistantInformation(NgcX2Sap::AssistantInformationForSplitting info){ //sjkang
...
void
NrRlcUm::ReassembleOutsideWindow (void)
{
...
void
NrRlcUm::ReassembleSnInterval (SequenceNumber10 lowSeqNumber, SequenceNumber10 highSeqNumber)
{
...
void
NrRlcUm::DoReportBufferStatus (void)
{
  Time holDelay (0);
  ...
  TxQueuingDelay = holDelay.GetSeconds();
...
void
NrRlcUm::ExpireReorderingTimer (void)
{
...
//int ccc=10;
void
NrRlcUm::CalculatePathThroughput (std::ofstream *stream) // woody
{
	this->stream = stream;
...
/*if (now.GetSeconds()>= m_simulationTime.GetSeconds()+1.0 && countAtRlc == 0 ){
  ...
  *stream << now.GetSeconds () << "\t" << cur << "\t"<< (double)((TotalPackets*8)/TotalTime)/1e6 <<std::endl;
  ...
  //if(isEnbaleMeasuring)
 // std::cout << this<<"\t"<<sumPacketSize<< std::endl;
// m_Tput_Calculator = Simulator::Schedule (MilliSeconds (100), &NrRlcUm::CalculatePathThroughput,this , stream);
  isEnbaleMeasuring =true;
  m_Tput_Calculator.Cancel();
  //ccc+=5;*/
...
NrRlcUm::CalculateThroughput(){
	...
	/*if (now.GetSeconds()>= m_simulationTime.GetSeconds()+1.0 && countAtRlc == 0 ){
	  ...
	  *stream << now.GetSeconds () << "\t" << cur << "\t"<<(double)((TotalPackets*8)/TotalTime)/1e6 <<std::endl;
	  lastSumPacketSize = sumPacketSize;
	  //if(isEnbaleMeasuring)
	  Simulator::Schedule (MilliSeconds (100), &NrRlcUm::CalculatePathThroughput,this , stream);
}
void
NrRlcUm::RecordingQueueStatistics(){//sjkang1116

	*measuringQusizeQueueDelayStream <<Simulator::Now().GetSeconds()<<"\t"<<TxQueueSize<<"\t"
			<<TxQueuingDelay<<std::endl;
Simulator::Schedule(MilliSeconds(1.0),&NrRlcUm::RecordingQueueStatistics,this);
}
void
NrRlcUm::DoRequestAssistantInfo(){
	 NgcX2Sap::AssistantInformationForSplitting info;
			            std::cout << "will send NR assistant info from NrRlcUmLowLat" <<std::endl;
			            RecordingQueueStatistics();
			            Simulator::Schedule(MilliSeconds(10),&NrRlcUm::DoSendAssistantInformation,this, info);
