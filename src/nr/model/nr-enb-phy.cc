...
NS_LOG_COMPONENT_DEFINE ("NrEnbPhy");

NS_OBJECT_ENSURE_REGISTERED (NrEnbPhy);
...
class EnbMemberNrEnbPhySapProvider : public NrEnbPhySapProvider
{
public:
  EnbMemberNrEnbPhySapProvider (NrEnbPhy* phy);

  // inherited from NrEnbPhySapProvider
  virtual void SendMacPdu (Ptr<Packet> p);
  virtual void SetBandwidth (uint8_t ulBandwidth, uint8_t dlBandwidth);
  virtual void SetCellId (uint16_t cellId);
  virtual void SendNrControlMessage (Ptr<NrControlMessage> msg);
...
EnbMemberNrEnbPhySapProvider::EnbMemberNrEnbPhySapProvider (NrEnbPhy* phy) : m_phy (phy)
{

}

void
EnbMemberNrEnbPhySapProvider::SendMacPdu (Ptr<Packet> p)
{
  m_phy->DoSendMacPdu (p);
}

void
EnbMemberNrEnbPhySapProvider::SetBandwidth (uint8_t ulBandwidth, uint8_t dlBandwidth)
{
  m_phy->DoSetBandwidth (ulBandwidth, dlBandwidth);
}

void
EnbMemberNrEnbPhySapProvider::SetCellId (uint16_t cellId)
{
  m_phy->DoSetCellId (cellId);
}

void
EnbMemberNrEnbPhySapProvider::SendNrControlMessage (Ptr<NrControlMessage> msg)
{
  m_phy->DoSendNrControlMessage (msg);
}

uint8_t
EnbMemberNrEnbPhySapProvider::GetMacChTtiDelay ()
{
...
NrEnbPhy::NrEnbPhy (Ptr<NrSpectrumPhy> dlPhy, Ptr<NrSpectrumPhy> ulPhy)
  : NrPhy (dlPhy, ulPhy),
    m_enbPhySapUser (0),
    m_enbCphySapUser (0),
    m_nrFrames (0),
...
{
  m_enbPhySapProvider = new EnbMemberNrEnbPhySapProvider (this);
  m_enbCphySapProvider = new MemberNrEnbCphySapProvider<NrEnbPhy> (this);
  m_harqPhyModule = Create <NrHarqPhy> ();
  m_downlinkSpectrumPhy->SetHarqPhyModule (m_harqPhyModule);
  m_uplinkSpectrumPhy->SetHarqPhyModule (m_harqPhyModule);
}

TypeId
NrEnbPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrEnbPhy")
    .SetParent<NrPhy> ()
    .SetGroupName("Nr")
    .AddConstructor<NrEnbPhy> ()
    .AddAttribute ("TxPower",
                   "Transmission power in dBm",
                   DoubleValue (30.0),
                   MakeDoubleAccessor (&NrEnbPhy::SetTxPower, 
                                       &NrEnbPhy::GetTxPower),
    ...
    .AddAttribute ("NoiseFigure",
                   ...
                   DoubleValue (5.0),
                   MakeDoubleAccessor (&NrEnbPhy::SetNoiseFigure, 
                                       &NrEnbPhy::GetNoiseFigure),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("MacToChannelDelay",
                   ...
                   UintegerValue (2),
                   MakeUintegerAccessor (&NrEnbPhy::SetMacChDelay, 
                                         &NrEnbPhy::GetMacChDelay),
                   MakeUintegerChecker<uint8_t> ())
    .AddTraceSource ("ReportUeSinr",
                     "Report UEs' averaged linear SINR",
                     MakeTraceSourceAccessor (&NrEnbPhy::m_reportUeSinr),
                     "ns3::NrEnbPhy::ReportUeSinrTracedCallback")
    .AddAttribute ("UeSinrSamplePeriod",
                   "The sampling period for reporting UEs' SINR stats.",
                   UintegerValue (1),  /// \todo In what unit is this?
                   MakeUintegerAccessor (&NrEnbPhy::m_srsSamplePeriod),
                   MakeUintegerChecker<uint16_t> ())
    .AddTraceSource ("ReportInterference",
                     "Report linear interference power per PHY RB",
                     MakeTraceSourceAccessor (&NrEnbPhy::m_reportInterferenceTrace),
                     "ns3::NrEnbPhy::ReportInterferenceTracedCallback")
    .AddAttribute ("InterferenceSamplePeriod",
                   "The sampling period for reporting interference stats",
                   UintegerValue (1),  /// \todo In what unit is this?
                   MakeUintegerAccessor (&NrEnbPhy::m_interferenceSamplePeriod),
                   MakeUintegerChecker<uint16_t> ())
    .AddTraceSource ("DlPhyTransmission",
                     "DL transmission PHY layer statistics.",
                     MakeTraceSourceAccessor (&NrEnbPhy::m_dlPhyTransmission),
                     "ns3::NrPhyTransmissionStatParameters::TracedCallback")
    .AddAttribute ("DlSpectrumPhy",
                   "The downlink NrSpectrumPhy associated to this NrPhy",
                   TypeId::ATTR_GET,
                   PointerValue (),
                   MakePointerAccessor (&NrEnbPhy::GetDlSpectrumPhy),
                   MakePointerChecker <NrSpectrumPhy> ())
    .AddAttribute ("UlSpectrumPhy",
                   "The uplink NrSpectrumPhy associated to this NrPhy",
                   TypeId::ATTR_GET,
                   PointerValue (),
                   MakePointerAccessor (&NrEnbPhy::GetUlSpectrumPhy),
                   MakePointerChecker <NrSpectrumPhy> ())
  ;
  return tid;
}
...
NrEnbPhy::DoInitialize ()
{
  ...
  if (haveNodeId)
    {
      Simulator::ScheduleWithContext (nodeId, Seconds (0), &NrEnbPhy::StartFrame, this);
    }
  else
    {
      Simulator::ScheduleNow (&NrEnbPhy::StartFrame, this);
    }
  Ptr<SpectrumValue> noisePsd = NrSpectrumValueHelper::CreateNoisePowerSpectralDensity (m_ulEarfcn, m_ulBandwidth, m_noiseFigure);
...
void
NrEnbPhy::SetNrEnbPhySapUser (NrEnbPhySapUser* s)
{
  m_enbPhySapUser = s;
}

NrEnbPhySapProvider*
NrEnbPhy::GetNrEnbPhySapProvider ()
{
  return (m_enbPhySapProvider);
}

void
NrEnbPhy::SetNrEnbCphySapUser (NrEnbCphySapUser* s)
{
  NS_LOG_FUNCTION (this);
  m_enbCphySapUser = s;
}
...
NrEnbPhy::GetNrEnbCphySapProvider ()
{
  NS_LOG_FUNCTION (this);
...
void
NrEnbPhy::SetTxPower (double pow)
{
...
int8_t
NrEnbPhy::DoGetReferenceSignalPower () const
{
  NS_LOG_FUNCTION (this);
  return m_txPower;
}
...
void
NrEnbPhy::SetNoiseFigure (double nf)
{
...
void
NrEnbPhy::SetMacChDelay (uint8_t delay)
{
  NS_LOG_FUNCTION (this);
  ...
  for (int i = 0; i < m_macChTtiDelay; i++)
    {
      ...
      std::list<Ptr<NrControlMessage> > l;
      m_controlMessagesQueue.push_back (l);
      std::list<UlDciNrControlMessage> l1;
...
uint8_t
NrEnbPhy::GetMacChDelay (void) const
{
  return (m_macChTtiDelay);
}

Ptr<NrSpectrumPhy>
NrEnbPhy::GetDlSpectrumPhy () const
{
  return m_downlinkSpectrumPhy;
}

Ptr<NrSpectrumPhy>
NrEnbPhy::GetUlSpectrumPhy () const
{
  return m_uplinkSpectrumPhy;
}

bool
NrEnbPhy::AddUePhy (uint16_t rnti)
{
...
bool
NrEnbPhy::DeleteUePhy (uint16_t rnti)
{
  NS_LOG_FUNCTION (this << rnti);
  std::set <uint16_t>::iterator it;
  it = m_ueAttached.find (rnti);
...
void
NrEnbPhy::DoSendMacPdu (Ptr<Packet> p)
{
  NS_LOG_FUNCTION (this);
  SetMacPdu (p);
}
...
void
NrEnbPhy::PhyPduReceived (Ptr<Packet> p)
{
  NS_LOG_FUNCTION (this);
...
void
NrEnbPhy::SetDownlinkSubChannels (std::vector<int> mask)
{
  NS_LOG_FUNCTION (this);
...
void
NrEnbPhy::SetDownlinkSubChannelsWithPowerAllocation (std::vector<int> mask)
{
  NS_LOG_FUNCTION (this);
  m_listOfDownlinkSubchannel = mask;
...
std::vector<int>
NrEnbPhy::GetDownlinkSubChannels (void)
{
  NS_LOG_FUNCTION (this);
...
void
NrEnbPhy::GeneratePowerAllocationMap (uint16_t rnti, int rbId)
{
  NS_LOG_FUNCTION (this);
...
Ptr<SpectrumValue>
NrEnbPhy::CreateTxPowerSpectralDensity ()
{
  NS_LOG_FUNCTION (this);

  Ptr<SpectrumValue> psd = NrSpectrumValueHelper::CreateTxPowerSpectralDensity (m_dlEarfcn, m_dlBandwidth, m_txPower, GetDownlinkSubChannels ());

  return psd;
}

Ptr<SpectrumValue>
NrEnbPhy::CreateTxPowerSpectralDensityWithPowerAllocation ()
{
  NS_LOG_FUNCTION (this);

  Ptr<SpectrumValue> psd = NrSpectrumValueHelper::CreateTxPowerSpectralDensity (m_dlEarfcn, m_dlBandwidth, m_txPower, m_dlPowerAllocationMap, GetDownlinkSubChannels ());
...
void
NrEnbPhy::CalcChannelQualityForUe (std::vector <double> sinr, Ptr<NrSpectrumPhy> ue)
{
  NS_LOG_FUNCTION (this);
}
...
void
NrEnbPhy::DoSendNrControlMessage (Ptr<NrControlMessage> msg)
{
...
void
NrEnbPhy::ReceiveNrControlMessage (Ptr<NrControlMessage> msg)
{
  NS_FATAL_ERROR ("Obsolete function");
  NS_LOG_FUNCTION (this << msg);
  m_enbPhySapUser->ReceiveNrControlMessage (msg);
}

void
NrEnbPhy::ReceiveNrControlMessageList (std::list<Ptr<NrControlMessage> > msgList)
{
  NS_LOG_FUNCTION (this);
  std::list<Ptr<NrControlMessage> >::iterator it;
  for (it = msgList.begin (); it != msgList.end (); it++)
    {
      switch ((*it)->GetMessageType ())
        ...
        case NrControlMessage::RACH_PREAMBLE:
          {
            Ptr<RachPreambleNrControlMessage> rachPreamble = DynamicCast<RachPreambleNrControlMessage> (*it);
        ...
        case NrControlMessage::DL_CQI:
          {
            ...
            if (m_ueAttached.find (dlcqi.m_rnti) != m_ueAttached.end ())
              {
                m_enbPhySapUser->ReceiveNrControlMessage (*it);
              }
          }
          break;
        ...
        case NrControlMessage::DL_HARQ:
          ...
          break;
        default:
          NS_FATAL_ERROR ("Unexpected NrControlMessage type");
          break;
        }
    }
}
...
NrEnbPhy::StartFrame (void)
{
  ...
  Ptr<MibNrControlMessage> mibMsg = Create<MibNrControlMessage> ();
  mibMsg->SetMib (m_mib);
...
NrEnbPhy::StartSubFrame (void)
{
  ...
  if ((m_nrSubFrames == 6) && ((m_nrFrames % 2) == 1))
    {
      Ptr<Sib1NrControlMessage> msg = Create<Sib1NrControlMessage> ();
      msg->SetSib1 (m_sib1);
  ...
  std::list<UlDciNrControlMessage> uldcilist = DequeueUlDci ();
  std::list<UlDciNrControlMessage>::iterator dciIt = uldcilist.begin ();
  ...
  for (dciIt = uldcilist.begin (); dciIt!=uldcilist.end (); dciIt++)
    {
      ...
      if (it2 == m_ueAttached.end ())
        ...
        {
          // send info of TB to NrSpectrumPhy 
          ...
          m_uplinkSpectrumPhy->AddExpectedTb ((*dciIt).GetDci ().m_rnti, (*dciIt).GetDci ().m_ndi, (*dciIt).GetDci ().m_tbSize, (*dciIt).GetDci ().m_mcs, rbMap, 0 /* always SISO*/, 0 /* no HARQ proc id in UL*/, 0 /*evaluated by NrSpectrumPhy*/, false /* UL*/);
  ...
  std::list<Ptr<NrControlMessage> > ctrlMsg = GetControlMessages ();
  m_dlDataRbMap.clear ();
  ...
  if (ctrlMsg.size () > 0)
    {
      ...
      while (it != ctrlMsg.end ())
        {
          Ptr<NrControlMessage> msg = (*it);
          if (msg->GetMessageType () == NrControlMessage::DL_DCI)
            {
              Ptr<DlDciNrControlMessage> dci = DynamicCast<DlDciNrControlMessage> (msg);
              ...
              for (uint8_t i = 0; i < dci->GetDci ().m_mcs.size (); i++)
                {
                  NrPhyTransmissionStatParameters params;
            ...
            {
              Ptr<UlDciNrControlMessage> dci = DynamicCast<UlDciNrControlMessage> (msg);
              QueueUlDci (*dci);
            }
          else if (msg->GetMessageType () == NrControlMessage::RAR)
            {
              Ptr<RarNrControlMessage> rarMsg = DynamicCast<RarNrControlMessage> (msg);
              for (std::list<RarNrControlMessage::Rar>::const_iterator it = rarMsg->RarListBegin (); it != rarMsg->RarListEnd (); ++it)
                {
  ...
  Simulator::Schedule (Seconds (GetTti ()),
                       &NrEnbPhy::EndSubFrame,
                       this);

}

void
NrEnbPhy::SendControlChannels (std::list<Ptr<NrControlMessage> > ctrlMsgList)
{
...
void
NrEnbPhy::SendDataChannels (Ptr<PacketBurst> pb)
{
...
void
NrEnbPhy::EndSubFrame (void)
{
  NS_LOG_FUNCTION (this << Simulator::Now ().GetSeconds ());
  if (m_nrSubFrames == 10)
    {
      Simulator::ScheduleNow (&NrEnbPhy::EndFrame, this);
    }
  else
    {
      Simulator::ScheduleNow (&NrEnbPhy::StartSubFrame, this);
    }
}


void
NrEnbPhy::EndFrame (void)
{
  NS_LOG_FUNCTION (this << Simulator::Now ().GetSeconds ());
  Simulator::ScheduleNow (&NrEnbPhy::StartFrame, this);
}
...
void 
NrEnbPhy::GenerateCtrlCqiReport (const SpectrumValue& sinr)
{
...
void
NrEnbPhy::ReportInterference (const SpectrumValue& interf)
{
...
void
NrEnbPhy::ReportRsReceivedPower (const SpectrumValue& power)
{
  // not used by eNB
}
...
//       NS_LOG_DEBUG ("ULCQI RB " << i << " value " << sinrdb);
      ...
      int16_t sinrFp = NrFfConverter::double2fpS11dot3 (sinrdb);
...
void
NrEnbPhy::DoSetBandwidth (uint8_t ulBandwidth, uint8_t dlBandwidth)
{
...
void 
NrEnbPhy::DoSetEarfcn (uint16_t ulEarfcn, uint16_t dlEarfcn)
{
...
void 
NrEnbPhy::DoAddUe (uint16_t rnti)
{
  NS_LOG_FUNCTION (this << rnti);
...
void 
NrEnbPhy::DoRemoveUe (uint16_t rnti)
{
  NS_LOG_FUNCTION (this << rnti);
...
void
NrEnbPhy::DoSetPa (uint16_t rnti, double pa)
{
  NS_LOG_FUNCTION (this << rnti);
...
void
NrEnbPhy::CreateSrsReport (uint16_t rnti, double srs)
{
...
void
NrEnbPhy::DoSetTransmissionMode (uint16_t  rnti, uint8_t txMode)
{
...
void
NrEnbPhy::QueueUlDci (UlDciNrControlMessage m)
{
  NS_LOG_FUNCTION (this);
  m_ulDciQueue.at (UL_PUSCH_TTIS_DELAY - 1).push_back (m);
}

std::list<UlDciNrControlMessage>
NrEnbPhy::DequeueUlDci (void)
{
  NS_LOG_FUNCTION (this);
  if (m_ulDciQueue.at (0).size ()>0)
    {
      std::list<UlDciNrControlMessage> ret = m_ulDciQueue.at (0);
      m_ulDciQueue.erase (m_ulDciQueue.begin ());
      std::list<UlDciNrControlMessage> l;
  ...
  else
    {
      ...
      std::list<UlDciNrControlMessage> l;
      m_ulDciQueue.push_back (l);
      std::list<UlDciNrControlMessage> emptylist;
      return (emptylist);
    }
}

void
NrEnbPhy::DoSetSrsConfigurationIndex (uint16_t  rnti, uint16_t srcCi)
{
  NS_LOG_FUNCTION (this);
...
void 
NrEnbPhy::DoSetMasterInformationBlock (NrRrcSap::MasterInformationBlock mib)
{
  NS_LOG_FUNCTION (this);
  m_mib = mib;
}
...
void
NrEnbPhy::DoSetSystemInformationBlockType1 (NrRrcSap::SystemInformationBlockType1 sib1)
{
  NS_LOG_FUNCTION (this);
  m_sib1 = sib1;
}
...
void
NrEnbPhy::SetHarqPhyModule (Ptr<NrHarqPhy> harq)
{
