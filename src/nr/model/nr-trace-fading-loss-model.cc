...
NS_LOG_COMPONENT_DEFINE ("NrTraceFadingLossModel");

NS_OBJECT_ENSURE_REGISTERED (NrTraceFadingLossModel);
  


NrTraceFadingLossModel::NrTraceFadingLossModel ()
...
TypeId
NrTraceFadingLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrTraceFadingLossModel")
    .SetParent<SpectrumPropagationLossModel> ()
    .SetGroupName("Nr")
    .AddConstructor<NrTraceFadingLossModel> ()
    .AddAttribute ("TraceFilename",
                   "Name of file to load a trace from.",
                   StringValue (""),
                   MakeStringAccessor (&NrTraceFadingLossModel::SetTraceFileName),
                   MakeStringChecker ())
    .AddAttribute ("TraceLength",
                  "The total length of the fading trace (default value 10 s.)",
                  TimeValue (Seconds (10.0)),
                  MakeTimeAccessor (&NrTraceFadingLossModel::SetTraceLength),
                  MakeTimeChecker ())
    .AddAttribute ("SamplesNum",
                  "The number of samples the trace is made of (default 10000)",
                   UintegerValue (10000),
                   MakeUintegerAccessor (&NrTraceFadingLossModel::m_samplesNum),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("WindowSize",
                  "The size of the window for the fading trace (default value 0.5 s.)",
                  TimeValue (Seconds (0.5)),
                  MakeTimeAccessor (&NrTraceFadingLossModel::m_windowSize),
                  MakeTimeChecker ())
    .AddAttribute ("RbNum",
                    "The number of RB the trace is made of (default 100)",
                    UintegerValue (100),
                   MakeUintegerAccessor (&NrTraceFadingLossModel::m_rbNum),
                   MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("RngStreamSetSize",
                    "The number of RNG streams reserved for the fading model. The maximum number of streams that are needed for an NR FDD scenario is 2 * numUEs * numeNBs.",
                    UintegerValue (200000),
                   MakeUintegerAccessor (&NrTraceFadingLossModel::m_streamSetSize),
...
void
NrTraceFadingLossModel::SetTraceFileName (std::string fileName)
{
...
void 
NrTraceFadingLossModel::SetTraceLength (Time t)
{
  m_traceLength = t;
}

void 
NrTraceFadingLossModel::DoInitialize ()
{
  LoadTrace ();
}
...
void
NrTraceFadingLossModel::LoadTrace ()
{
...
Ptr<SpectrumValue>
NrTraceFadingLossModel::DoCalcRxPowerSpectralDensity (
...
int64_t
NrTraceFadingLossModel::AssignStreams (int64_t stream)
{
