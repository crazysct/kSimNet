...
NS_LOG_COMPONENT_DEFINE ("NrFfrDistributedAlgorithm");

NS_OBJECT_ENSURE_REGISTERED (NrFfrDistributedAlgorithm);


NrFfrDistributedAlgorithm::NrFfrDistributedAlgorithm ()
  : m_ffrSapUser (0),
    m_ffrRrcSapUser (0)
{
  NS_LOG_FUNCTION (this);
  m_ffrSapProvider = new MemberNrFfrSapProvider<NrFfrDistributedAlgorithm> (this);
  m_ffrRrcSapProvider = new MemberNrFfrRrcSapProvider<NrFfrDistributedAlgorithm> (this);
}
...
void
NrFfrDistributedAlgorithm::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
TypeId
NrFfrDistributedAlgorithm::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::NrFfrDistributedAlgorithm")
    .SetParent<NrFfrAlgorithm> ()
    .SetGroupName("Nr")
    .AddConstructor<NrFfrDistributedAlgorithm> ()
    .AddAttribute ("CalculationInterval",
                   "Time interval between calculation of Edge sub-band, Default value 1 second",
                   TimeValue (Seconds (1)),
                   MakeTimeAccessor (&NrFfrDistributedAlgorithm::m_calculationInterval),
                   MakeTimeChecker ())
    .AddAttribute ("RsrqThreshold",
                   "If the RSRQ of is worse than this threshold, UE should be served in Edge sub-band",
                   UintegerValue (20),
                   MakeUintegerAccessor (&NrFfrDistributedAlgorithm::m_egdeSubBandRsrqThreshold),
    ...
    .AddAttribute ("RsrpDifferenceThreshold",
                   ...
                   UintegerValue (20),
                   MakeUintegerAccessor (&NrFfrDistributedAlgorithm::m_rsrpDifferenceThreshold),
                   MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("CenterPowerOffset",
                   "PdschConfigDedicated::Pa value for Edge Sub-band, default value dB0",
                   UintegerValue (5),
                   MakeUintegerAccessor (&NrFfrDistributedAlgorithm::m_centerPowerOffset),
                   MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("EdgePowerOffset",
                   "PdschConfigDedicated::Pa value for Edge Sub-band, default value dB0",
                   UintegerValue (5),
                   MakeUintegerAccessor (&NrFfrDistributedAlgorithm::m_edgePowerOffset),
                   MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("EdgeRbNum",
                   "Number of RB that can be used in edge Sub-band",
                   UintegerValue (8),
                   MakeUintegerAccessor (&NrFfrDistributedAlgorithm::m_edgeRbNum),
                   MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("CenterAreaTpc",
                   ...
                   UintegerValue (1),
                   MakeUintegerAccessor (&NrFfrDistributedAlgorithm::m_centerAreaTpc),
                   MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("EdgeAreaTpc",
                   ...
                   "TS36.213 Table 5.1.1.1-2",
                   UintegerValue (1),
                   MakeUintegerAccessor (&NrFfrDistributedAlgorithm::m_edgeAreaTpc),
                   MakeUintegerChecker<uint8_t> ())
...
void
NrFfrDistributedAlgorithm::SetNrFfrSapUser (NrFfrSapUser* s)
{
...
NrFfrSapProvider*
NrFfrDistributedAlgorithm::GetNrFfrSapProvider ()
{
  NS_LOG_FUNCTION (this);
...
void
NrFfrDistributedAlgorithm::SetNrFfrRrcSapUser (NrFfrRrcSapUser* s)
{
  NS_LOG_FUNCTION (this << s);
  m_ffrRrcSapUser = s;
}
...
NrFfrRrcSapProvider*
NrFfrDistributedAlgorithm::GetNrFfrRrcSapProvider ()
{
  NS_LOG_FUNCTION (this);
...
void
NrFfrDistributedAlgorithm::DoInitialize ()
{
  NS_LOG_FUNCTION (this);
  ...
  reportConfigA4.reportInterval = NrRrcSap::ReportConfigEutra::MS120;
  ...
  m_calculationEvent = Simulator::ScheduleNow (&NrFfrDistributedAlgorithm::Calculate, this);

}

void
NrFfrDistributedAlgorithm::Reconfigure ()
{
  NS_LOG_FUNCTION (this);
  if (m_frCellTypeId != 0)
    {
...
void
NrFfrDistributedAlgorithm::SetDownlinkConfiguration (uint16_t cellId, uint8_t bandwidth)
{
  NS_LOG_FUNCTION (this);
}

void
NrFfrDistributedAlgorithm::SetUplinkConfiguration (uint16_t cellId, uint8_t bandwidth)
{
  NS_LOG_FUNCTION (this);
}

void
NrFfrDistributedAlgorithm::InitializeDownlinkRbgMaps ()
{
  NS_LOG_FUNCTION (this);
  m_dlRbgMap.clear ();
  int rbgSize = GetRbgSize (m_dlBandwidth);
...
void
NrFfrDistributedAlgorithm::InitializeUplinkRbgMaps ()
{
  NS_LOG_FUNCTION (this);
  m_ulRbgMap.clear ();
...
std::vector <bool>
NrFfrDistributedAlgorithm::DoGetAvailableDlRbg ()
{
  NS_LOG_FUNCTION (this);
...
bool
NrFfrDistributedAlgorithm::DoIsDlRbgAvailableForUe (int rbgId, uint16_t rnti)
{
  NS_LOG_FUNCTION (this);
...
std::vector <bool>
NrFfrDistributedAlgorithm::DoGetAvailableUlRbg ()
{
  NS_LOG_FUNCTION (this);
...
bool
NrFfrDistributedAlgorithm::DoIsUlRbgAvailableForUe (int rbId, uint16_t rnti)
{
  NS_LOG_FUNCTION (this);
...
void
NrFfrDistributedAlgorithm::DoReportUlCqiInfo (std::map <uint16_t, std::vector <double> > ulCqiMap)
{
  NS_LOG_FUNCTION (this);
  NS_LOG_WARN ("Method should not be called, because it is empty");
}
...
uint8_t
NrFfrDistributedAlgorithm::DoGetTpc (uint16_t rnti)
{
  NS_LOG_FUNCTION (this);
...
uint8_t
NrFfrDistributedAlgorithm::DoGetMinContinuousUlBandwidth ()
{
  NS_LOG_FUNCTION (this);
...
void
NrFfrDistributedAlgorithm::DoReportUeMeas (uint16_t rnti,
  ...
  if (measResults.measId == m_rsrqMeasId)
  ...
  else if (measResults.measId == m_rsrpMeasId)
    {
      ...
      if (measResults.haveMeasResultNeighCells
        ...
        {
          for (std::list <NrRrcSap::MeasResultEutra>::iterator it = measResults.measResultListEutra.begin ();
...
void
NrFfrDistributedAlgorithm::Calculate ()
{
  NS_LOG_FUNCTION (this);
  m_calculationEvent = Simulator::Schedule (m_calculationInterval, &NrFfrDistributedAlgorithm::Calculate, this);
...
void
NrFfrDistributedAlgorithm::SendLoadInformation (uint16_t targetCellId)
{
  NS_LOG_FUNCTION (this);
...
void
NrFfrDistributedAlgorithm::DoRecvLoadInformation (NgcX2Sap::LoadInformationParams params)
{
  NS_LOG_FUNCTION (this);
...
void
NrFfrDistributedAlgorithm::UpdateNeighbourMeasurements (uint16_t rnti,
                                                         uint16_t cellId,
                                                         uint8_t rsrp,
                                                         uint8_t rsrq)
{
