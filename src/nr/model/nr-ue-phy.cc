...
class UeMemberNrUePhySapProvider : public NrUePhySapProvider
{
public:
  UeMemberNrUePhySapProvider (NrUePhy* phy);

  // inherited from NrPhySapProvider
...
UeMemberNrUePhySapProvider::UeMemberNrUePhySapProvider (NrUePhy* phy) : m_phy (phy)
{

}

void
UeMemberNrUePhySapProvider::SendMacPdu (Ptr<Packet> p)
{
  m_phy->DoSendMacPdu (p);
}

void
UeMemberNrUePhySapProvider::SendNrControlMessage (Ptr<NrControlMessage> msg)
{
  m_phy->DoSendNrControlMessage (msg);
}

void
UeMemberNrUePhySapProvider::SendRachPreamble (uint32_t prachId, uint32_t raRnti)
{
...
static const std::string g_uePhyStateName[NrUePhy::NUM_STATES] =
{
  "CELL_SEARCH",
  "SYNCHRONIZED"
};
 ...
 */
static inline const std::string & ToString (NrUePhy::State s)
{
...
NrUePhy::NrUePhy (Ptr<NrSpectrumPhy> dlPhy, Ptr<NrSpectrumPhy> ulPhy)
  : NrPhy (dlPhy, ulPhy),
    ...
    m_rsrpSinrSampleCounter (0)
{
  ...
  m_uePhySapProvider = new UeMemberNrUePhySapProvider (this);
...
NrUePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrUePhy")
    ...
    .AddAttribute ("TxPower",
                   ...
                   DoubleValue (10.0),
                   MakeDoubleAccessor (&NrUePhy::SetTxPower, 
                                       &NrUePhy::GetTxPower),
    ...
    .AddTraceSource ("StateTransition",
                     ...
                     MakeTraceSourceAccessor (&NrUePhy::m_stateTransitionTrace),
                     "ns3::NrUePhy::StateTracedCallback")
    .AddAttribute ("EnableUplinkPowerControl",
                   "If true, Uplink Power Control will be enabled.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&NrUePhy::m_enableUplinkPowerControl),
                   MakeBooleanChecker ())
  ;
  return tid;
}
...
NrUePhy::DoInitialize ()
{
  ...
  if (haveNodeId)
    {
      Simulator::ScheduleWithContext (nodeId, Seconds (0), &NrUePhy::SubframeIndication, this, 1, 1);
    }
  else
    {
      Simulator::ScheduleNow (&NrUePhy::SubframeIndication, this, 1, 1);
    }  
  NrPhy::DoInitialize ();
}

void
NrUePhy::SetNrUePhySapUser (NrUePhySapUser* s)
{
  NS_LOG_FUNCTION (this);
  m_uePhySapUser = s;
}
...
void
NrUePhy::SetNrUeCphySapUser (NrUeCphySapUser* s)
{
  NS_LOG_FUNCTION (this);
  m_ueCphySapUser = s;
}
...
NrUeCphySapProvider*
NrUePhy::GetNrUeCphySapProvider ()
{
  NS_LOG_FUNCTION (this);
...
void
NrUePhy::SetNoiseFigure (double nf)
{
...
uint8_t
NrUePhy::GetMacChDelay (void) const
{
  return (m_macChTtiDelay);
}

Ptr<NrSpectrumPhy>
NrUePhy::GetDlSpectrumPhy () const
{
  return m_downlinkSpectrumPhy;
}

Ptr<NrSpectrumPhy>
NrUePhy::GetUlSpectrumPhy () const
{
  return m_uplinkSpectrumPhy;
}

void
NrUePhy::DoSendMacPdu (Ptr<Packet> p)
{
  NS_LOG_FUNCTION (this);
...
void
NrUePhy::PhyPduReceived (Ptr<Packet> p)
{
...
void
NrUePhy::SetSubChannelsForReception (std::vector <int> mask)
{
  NS_LOG_FUNCTION (this);
...
std::vector <int>
NrUePhy::GetSubChannelsForTransmission ()
{
  NS_LOG_FUNCTION (this);
...
std::vector <int>
NrUePhy::GetSubChannelsForReception ()
{
  NS_LOG_FUNCTION (this);
...
Ptr<SpectrumValue>
NrUePhy::CreateTxPowerSpectralDensity ()
{
  NS_LOG_FUNCTION (this);
  NrSpectrumValueHelper psdHelper;
...
void
NrUePhy::GenerateCtrlCqiReport (const SpectrumValue& sinr)
{
  NS_LOG_FUNCTION (this);
...
void
NrUePhy::GenerateCqiRsrpRsrq (const SpectrumValue& sinr)
{
  ...
  if (m_dlConfigured && m_ulConfigured && (m_rnti > 0))
    {
      ...
      if (Simulator::Now () > m_p10CqiLast + m_p10CqiPeriocity)
        {
          //Ptr<NrUeNetDevice> thisDevice = GetDevice ()->GetObject<NrUeNetDevice> ();
          Ptr<DlCqiNrControlMessage> msg = CreateDlCqiFeedbackMessage (sinr);
          if (msg)
            {
              DoSendNrControlMessage (msg);
            }
...
} // end of void NrUePhy::GenerateCtrlCqiReport (const SpectrumValue& sinr)

void
NrUePhy::GenerateDataCqiReport (const SpectrumValue& sinr)
{
  // Not used by UE, CQI are based only on RS
}

void
NrUePhy::GenerateMixedCqiReport (const SpectrumValue& sinr)
{
  NS_LOG_FUNCTION (this);
...
void
NrUePhy::ReportInterference (const SpectrumValue& interf)
{
...
void
NrUePhy::ReportDataInterference (const SpectrumValue& interf)
{
  NS_LOG_FUNCTION (this << interf);
...
void
NrUePhy::ReportRsReceivedPower (const SpectrumValue& power)
{
  ...
  if (Simulator::Now () > m_p10CqiLast + m_p10CqiPeriocity)
    {
      ...
      int nLayer = NrTransmissionModesLayers::TxMode2LayerNum (m_transmissionMode);
...
void
NrUePhy::DoSendNrControlMessage (Ptr<NrControlMessage> msg)
{
...
void 
NrUePhy::DoSendRachPreamble (uint32_t raPreambleId, uint32_t raRnti)
{
  NS_LOG_FUNCTION (this << raPreambleId);

  // unlike other control messages, RACH preamble is sent ASAP
  Ptr<RachPreambleNrControlMessage> msg = Create<RachPreambleNrControlMessage> ();
...
void
NrUePhy::ReceiveNrControlMessageList (std::list<Ptr<NrControlMessage> > msgList)
{
  NS_LOG_FUNCTION (this);
...
//             NS_LOG_DEBUG(this << " RNTI " << m_rnti << " RBG " << i << " DL-DCI allocated PRB " << (i*GetRbgSize()) + k);
      ...
      else if (msg->GetMessageType () == NrControlMessage::UL_DCI)
        {
          ...
          m_uePhySapUser->ReceiveNrControlMessage (msg);
        }
      else if (msg->GetMessageType () == NrControlMessage::RAR)
        {
          Ptr<RarNrControlMessage> rarMsg = DynamicCast<RarNrControlMessage> (msg);
          NS_LOG_INFO("Rx rar in NrUePhy, rarMsg rnti " << rarMsg->GetRaRnti() << " m_raRnti " << m_raRnti);
      ...
      else if (msg->GetMessageType () == NrControlMessage::MIB)
        {
          ...
          NS_ASSERT (m_cellId > 0);
          Ptr<MibNrControlMessage> msg2 = DynamicCast<MibNrControlMessage> (msg);
      ...
      else if (msg->GetMessageType () == NrControlMessage::SIB1)
        {
          ...
          NS_ASSERT (m_cellId > 0);
          Ptr<Sib1NrControlMessage> msg2 = DynamicCast<Sib1NrControlMessage> (msg);
...
void
NrUePhy::ReceivePss (uint16_t cellId, Ptr<SpectrumValue> p)
{
...
} // end of void NrUePhy::ReceivePss (uint16_t cellId, Ptr<SpectrumValue> p)


void
NrUePhy::QueueSubChannelsForTransmission (std::vector <int> rbMap)
{
...
void
NrUePhy::SubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  NS_LOG_FUNCTION (this << frameNo << subframeNo);

  NS_ASSERT_MSG (frameNo > 0, "the SRS index check code assumes that frameNo starts at 1");
  NS_LOG_INFO ("Nr UE " << m_rnti << " frame " << frameNo << " subframe " << subframeNo);
  ...
  Simulator::Schedule (Seconds (GetTti ()), &NrUePhy::SubframeIndication, this, frameNo, subframeNo);
}
...
void
NrUePhy::DoSetDlBandwidth (uint8_t dlBandwidth)
{
  ...
  if (m_dlBandwidth != dlBandwidth or !m_dlConfigured)
    {
      ...
      m_noisePsd = NrSpectrumValueHelper::CreateNoisePowerSpectralDensity (m_dlEarfcn, m_dlBandwidth, m_noiseFigure);
...
void 
NrUePhy::SetTxMode1Gain (double gain)
{
  SetTxModeGain (1, gain);
}

void 
NrUePhy::SetTxMode2Gain (double gain)
{
  SetTxModeGain (2, gain);
}

void 
NrUePhy::SetTxMode3Gain (double gain)
{
  SetTxModeGain (3, gain);
}

void 
NrUePhy::SetTxMode4Gain (double gain)
{
  SetTxModeGain (4, gain);
}

void 
NrUePhy::SetTxMode5Gain (double gain)
{
  SetTxModeGain (5, gain);
}

void 
NrUePhy::SetTxMode6Gain (double gain)
{
  SetTxModeGain (6, gain);
}

void 
NrUePhy::SetTxMode7Gain (double gain)
{
...
void
NrUePhy::SetTxModeGain (uint8_t txMode, double gain)
{
  ...
  // forward the info to DL NrSpectrumPhy
...
NrUePhy::ReceiveNrDlHarqFeedback (NrDlInfoListElement_s m)
{
  ...
  Ptr<DlHarqFeedbackNrControlMessage> msg = Create<DlHarqFeedbackNrControlMessage> ();
  msg->SetDlHarqFeedback (m);
  SetControlMessages (msg);
}

void
NrUePhy::SetHarqPhyModule (Ptr<NrHarqPhy> harq)
{
...
void
NrUePhy::SwitchToState (State newState)
{
