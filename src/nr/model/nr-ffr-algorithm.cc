...
{
  static TypeId tid = TypeId ("ns3::NrFfrAlgorithm")
    .SetParent<Object> ()
    .SetGroupName("Nr")
    .AddAttribute ("FrCellTypeId",
                   ...
                   UintegerValue (0),
                   MakeUintegerAccessor (&NrFfrAlgorithm::SetFrCellTypeId,&NrFfrAlgorithm::GetFrCellTypeId),
...
void
NrFfrAlgorithm::SetUlBandwidth (uint8_t bw)
{
...
void
NrFfrAlgorithm::SetDlBandwidth (uint8_t bw)
{
  NS_LOG_FUNCTION (this << uint16_t (bw));
  switch (bw)
    {
...
void
NrFfrAlgorithm::DoSetBandwidth (uint8_t ulBandwidth, uint8_t dlBandwidth)
{
  NS_LOG_FUNCTION (this);
