  ...
  for (; it != drbToAddModList.end (); it++)
    {
      ...
      SerializeBoolean (it->is_mc_2); //sjkang0710
    }
}
  ...
  if (!measConfig.measObjectToAddModList.empty ())
    {
      ...
      for (std::list<NrRrcSap::MeasObjectToAddMod>::iterator it = measConfig.measObjectToAddModList.begin (); it != measConfig.measObjectToAddModList.end (); it++)
        {
          SerializeSequence (std::bitset<0> (), false);
          ...
          if (!it->measObjectEutra.cellsToAddModList.empty ())
            {
              ...
              for (std::list<NrRrcSap::CellsToAddMod>::iterator it2 = it->measObjectEutra.cellsToAddModList.begin (); it2 != it->measObjectEutra.cellsToAddModList.end (); it2++)
                {
                  SerializeSequence (std::bitset<0> (), false);
          ...
          if (!it->measObjectEutra.blackCellsToAddModList.empty () )
            {
              ...
              for (std::list<NrRrcSap::BlackCellsToAddMod>::iterator it2 = it->measObjectEutra.blackCellsToAddModList.begin (); it2 != it->measObjectEutra.blackCellsToAddModList.end (); it2++)
                {
                  SerializeSequence (std::bitset<0> (),false);
                  SerializeInteger (it2->cellIndex, 1, MAX_CELL_MEAS);
  ...
  if (!measConfig.reportConfigToAddModList.empty () )
    {
      ...
      for (std::list<NrRrcSap::ReportConfigToAddMod>::iterator it = measConfig.reportConfigToAddModList.begin (); it != measConfig.reportConfigToAddModList.end (); it++)
        {
          SerializeSequence (std::bitset<0> (), false);
          ...
          if (it->reportConfigEutra.reportQuantity == NrRrcSap::ReportConfigEutra::SAME_AS_TRIGGER_QUANTITY)
            {
              SerializeEnum (2, 0);
            }
          else
  ...
  if (!measConfig.measIdToAddModList.empty () )
    {
      ...
      for (std::list<NrRrcSap::MeasIdToAddMod>::iterator it = measConfig.measIdToAddModList.begin (); it != measConfig.measIdToAddModList.end (); it++)
        {
  ...
  for (int i = 0; i < n; i++)
    {
      ...
      bIterator = DeserializeBoolean(&drbToAddMod.is_mc_2,bIterator); //sjkang0710
  ...
  if (optionalFieldPresent[2])
    {
      ...
      if (sel == 0)
        {
          physicalConfigDedicated->soundingRsUlConfigDedicated.type = NrRrcSap::SoundingRsUlConfigDedicated::RESET;
        ...
        {
          physicalConfigDedicated->soundingRsUlConfigDedicated.type = NrRrcSap::SoundingRsUlConfigDedicated::SETUP;
...
RrcNrAsn1Header::Print (std::ostream &os, NrRrcSap::RadioResourceConfigDedicated radioResourceConfigDedicated) const
{
  ...
  std::list<NrRrcSap::DrbToAddMod>::iterator it2 = radioResourceConfigDedicated.drbToAddModList.begin ();
          ...
          if (reportConfigChoice == 0)
            {
              ...
              if (triggerTypeChoice == 1)
                {
                  ...
                  if (n == 0)
                    {
                      elem.reportConfigEutra.purpose = NrRrcSap::ReportConfigEutra::REPORT_STRONGEST_CELLS;
                    }
                  else
                    {
              ...
              if (n == 0)
                {
                  elem.reportConfigEutra.reportQuantity = NrRrcSap::ReportConfigEutra::SAME_AS_TRIGGER_QUANTITY;
                }
              else
                {
...
RrcConnectionRequestHeader::PreSerialize () const
{
  ...
  // Serialize amfid : AMFID ::= BIT STRING (SIZE (8))
  SerializeBitstring (m_amfid);
...
RrcConnectionRequestHeader::Deserialize (Buffer::Iterator bIterator)
{
  ...
  // Deserialize amfid
  bIterator = DeserializeBitstring (&m_amfid,bIterator);
...
void
RrcConnectionRequestHeader::SetMessage (NrRrcSap::RrcConnectionRequest msg)
{
  m_mTmsi = std::bitset<32> ((uint32_t)msg.ueIdentity);
  m_amfid = std::bitset<8> ((uint32_t)(msg.ueIdentity >> 32));
...
RrcConnectionRequestHeader::GetMessage () const
{
  ...
  msg.ueIdentity = (((uint64_t) m_amfid.to_ulong ()) << 32) | (m_mTmsi.to_ulong ());
  msg.isMc = (bool) m_spare[0];
  return msg;
}

std::bitset<8>
RrcConnectionRequestHeader::GetAmfid () const
{
  return m_amfid;
}
...
RrcConnectToMmWaveHeader::Print (std::ostream &os) const
{
  ...
  os<<"MmWaveId_2"<<m_mmWaveId_2 <<std::endl; //sjkang
}
...
RrcConnectToMmWaveHeader::PreSerialize () const
{
  ...
  // Serialize mmWaveId : AMFID ::= BIT STRING (SIZE (16))
  SerializeBitstring (m_mmWaveId);
  SerializeBitstring(m_mmWaveId_2); //sjkang
  // Finish serialization
  FinalizeSerialization ();
}
...
RrcConnectToMmWaveHeader::Deserialize (Buffer::Iterator bIterator)
{
  ...
  bIterator = DeserializeBitstring (&m_mmWaveId_2,bIterator); //sjkang
...
RrcConnectToMmWaveHeader::SetMessage (uint16_t mmWaveId, uint16_t mmWaveId_2)
{
  ...
  m_mmWaveId_2 = std::bitset<16> ((uint16_t)mmWaveId_2); //sjkang
  m_isDataSerialized = false;
}
...
{
	return (uint16_t)(m_mmWaveId_2.to_ulong()); //sjkang
}
...
void
RrcConnectionSetupHeader::SetMessage (NrRrcSap::RrcConnectionSetup msg)
{
...
void
RrcConnectionReconfigurationCompleteHeader::SetMessage (NrRrcSap::RrcConnectionReconfigurationCompleted msg)
{
  m_rrcTransactionIdentifier = msg.rrcTransactionIdentifier;
  m_isDataSerialized = false;
}
...
void
RrcConnectionSwitchHeader::SetMessage (NrRrcSap::RrcConnectionSwitch msg)
{
  m_msg = msg;
  m_isDataSerialized = false;
}

NrRrcSap::RrcConnectionSwitch
...
RrcConnectionReconfigurationHeader::PreSerialize () const
{
  ...
  SerializeInteger(m_handoverCase,0,3); //sjkang0417
  // Serialize criticalExtensions choice
  // 2 options, selected 0 (c1)
  SerializeChoice (2,0,false);
  ...
  //options.set(6,m_handoverCase); //sjkang0417
...
RrcConnectionReconfigurationHeader::Deserialize (Buffer::Iterator bIterator)
{
  ...
  int h; //sjkang
  bIterator = DeserializeInteger(&h,0,3,bIterator); //sjkang0417
  m_handoverCase = h; //sjkang
  // criticalExtensions
  int sel;
  bIterator = DeserializeChoice (2,false,&sel,bIterator);
  if (sel == 1)
  ...
  else if (sel == 0)
    {
      ...
      if (c1Chosen > 0)
        ...
        //  m_handoverCase = rrcConnRecOpts[0]; //sjkang0417
          if (m_haveMobilityControlInfo)
            {
              // mobilityControlInfo
...
{
  os << "rrcTransactionIdentifier: " << (int) m_rrcTransactionIdentifier << std::endl;
  os << "to send rrc connection reconfiguration message for Handover case " <<(int)m_handoverCase << std::endl; //sjkang0417
      ...
      if (!m_measConfig.measObjectToAddModList.empty ())
        {
          ...
          std::list<NrRrcSap::MeasObjectToAddMod>::iterator it = auxList.begin ();
          for (; it != auxList.end (); it++)
            {
              ...
              if (!it->measObjectEutra.cellsToAddModList.empty ())
                {
                  ...
                  std::list<NrRrcSap::CellsToAddMod> auxList = it->measObjectEutra.cellsToAddModList;
                  std::list<NrRrcSap::CellsToAddMod>::iterator it = auxList.begin ();
                  for (; it != auxList.end (); it++)
                    {
              ...
              if (!it->measObjectEutra.blackCellsToAddModList.empty ())
                {
                  ...
                  std::list<NrRrcSap::BlackCellsToAddMod> auxList = it->measObjectEutra.blackCellsToAddModList;
                  std::list<NrRrcSap::BlackCellsToAddMod>::iterator it = auxList.begin ();
                  for (; it != auxList.end (); it++)
                    {
                      os << "      cellIndex: " <<  (int)it->cellIndex << std::endl;
      ...
      if (!m_measConfig.reportConfigToAddModList.empty ())
        {
          ...
          std::list<NrRrcSap::ReportConfigToAddMod> auxList = m_measConfig.reportConfigToAddModList;
          std::list<NrRrcSap::ReportConfigToAddMod>::iterator it = auxList.begin ();
          for (; it != auxList.end (); it++)
            {
      ...
      if (!m_measConfig.measIdToAddModList.empty ())
        {
          ...
          std::list<NrRrcSap::MeasIdToAddMod>::iterator it = auxList.begin ();
          for (; it != auxList.end (); it++)
            {
...
void
RrcConnectionReconfigurationHeader::SetMessage (NrRrcSap::RrcConnectionReconfiguration msg)
{
  m_rrcTransactionIdentifier = msg.rrcTransactionIdentifier;
  ...
  m_radioResourceConfigDedicated = msg.radioResourceConfigDedicated;
  m_handoverCase = msg.HandoverCase; //sjkang
  m_isDataSerialized = false;
}
...
RrcConnectionReconfigurationHeader::GetMessage () const
{
  ...
  msg.radioResourceConfigDedicated = m_radioResourceConfigDedicated;
  msg.HandoverCase = m_handoverCase;
  return msg;
}
...
// jhlim
//////////////////// IdentityRequest,Response,RegistrationAccept,CompleteHeader class ////////////////////
...
RrcIdentityResponseHeader::RrcIdentityResponseHeader ()
{
}
RrcIdentityResponseHeader::~RrcIdentityResponseHeader ()
{
...
void
RrcIdentityResponseHeader::SetMessage (NrRrcSap::RrcIdentityResponse msg)
{
}
...
void
RrcRegistrationAcceptHeader::SetMessage (NrRrcSap::RrcRegistrationAccept msg)
{
}

RrcRegistrationCompleteHeader::RrcRegistrationCompleteHeader ()
{
}
RrcRegistrationCompleteHeader::~RrcRegistrationCompleteHeader ()
{
...
void
RrcRegistrationCompleteHeader::SetMessage (NrRrcSap::RrcRegistrationComplete msg)
{
}
...
void
RrcConnectionReestablishmentRequestHeader::SetMessage (NrRrcSap::RrcConnectionReestablishmentRequest msg)
{
...
void
RrcConnectionReestablishmentCompleteHeader::SetMessage (NrRrcSap::RrcConnectionReestablishmentComplete msg)
{
  m_rrcTransactionIdentifier = msg.rrcTransactionIdentifier;
  m_isDataSerialized = false;
}
...
void
RrcConnectionReestablishmentRejectHeader::SetMessage (NrRrcSap::RrcConnectionReestablishmentReject msg)
{
...
void
RrcConnectionReleaseHeader::SetMessage (NrRrcSap::RrcConnectionRelease msg)
{
...
MeasurementReportHeader::Print (std::ostream &os) const
{
  ...
  if (m_measurementReport.measResults.haveMeasResultNeighCells)
    {
      std::list<NrRrcSap::MeasResultEutra> measResultListEutra = m_measurementReport.measResults.measResultListEutra;
