...
double 
NrSpectrumValueHelper::GetCarrierFrequency (uint16_t earfcn)
{
...
double 
NrSpectrumValueHelper::GetDownlinkCarrierFrequency (uint16_t nDl)
{
  NS_LOG_FUNCTION (nDl);
...
double 
NrSpectrumValueHelper::GetUplinkCarrierFrequency (uint16_t nUl)
{
  NS_LOG_FUNCTION (nUl);
  for (uint16_t i = 0; i < NUM_EUTRA_BANDS; ++i)
    {
...
double 
NrSpectrumValueHelper::GetChannelBandwidth (uint8_t transmissionBandwidth)
{
...
{
  NrSpectrumModelId (uint16_t f, uint8_t b);
  uint16_t earfcn;
  uint8_t  bandwidth;
};

NrSpectrumModelId::NrSpectrumModelId (uint16_t f, uint8_t b)
  : earfcn (f), 
    bandwidth (b)
{
}
...
Ptr<SpectrumModel>
NrSpectrumValueHelper::GetSpectrumModel (uint16_t earfcn, uint8_t txBandwidthConfiguration)
{
  NS_LOG_FUNCTION (earfcn << (uint16_t) txBandwidthConfiguration);
  Ptr<SpectrumModel> ret;
  NrSpectrumModelId key (earfcn, txBandwidthConfiguration);
  std::map<NrSpectrumModelId, Ptr<SpectrumModel> >::iterator it = g_nrSpectrumModelMap.find (key);
  if (it != g_nrSpectrumModelMap.end ())
    {
      ret = it->second;
    }
  else
    {
      ...
      g_nrSpectrumModelMap.insert (std::pair<NrSpectrumModelId, Ptr<SpectrumModel> > (key, ret));
    }
...
Ptr<SpectrumValue> 
NrSpectrumValueHelper::CreateTxPowerSpectralDensity (uint16_t earfcn, uint8_t txBandwidthConfiguration, double powerTx, std::vector <int> activeRbs)
{
...
Ptr<SpectrumValue>
NrSpectrumValueHelper::CreateTxPowerSpectralDensity (uint16_t earfcn, uint8_t txBandwidthConfiguration, double powerTx, std::map<int, double> powerTxMap, std::vector <int> activeRbs)
{
...
Ptr<SpectrumValue>
NrSpectrumValueHelper::CreateNoisePowerSpectralDensity (uint16_t earfcn, uint8_t txBandwidthConfiguration, double noiseFigure)
{
...
Ptr<SpectrumValue>
NrSpectrumValueHelper::CreateNoisePowerSpectralDensity (double noiseFigureDb, Ptr<SpectrumModel> spectrumModel)
{
  NS_LOG_FUNCTION (noiseFigureDb << spectrumModel);


  // see "NR - From theory to practice"
