...
NrFlowId_t::NrFlowId_t ()
{
}

NrFlowId_t::NrFlowId_t (const uint16_t a, const uint8_t b)
  : m_rnti (a),
    m_lcId (b)
{
}
...
bool
operator == (const NrFlowId_t &a, const NrFlowId_t &b)
{
...
NrImsiLcidPair_t::NrImsiLcidPair_t ()
{
}

NrImsiLcidPair_t::NrImsiLcidPair_t (const uint64_t a, const uint8_t b)
  : m_imsi (a),
    m_lcId (b)
{
}
...
bool
operator == (const NrImsiLcidPair_t &a, const NrImsiLcidPair_t &b)
{
...
uint16_t
NrFfConverter::double2fpS11dot3 (double val)
{
...
double
NrFfConverter::fpS11dot3toDouble (uint16_t val)
{
...
double 
NrFfConverter::getMinFpS11dot3Value ()
{
...
static const uint32_t NrBufferSizeLevelBsrTable[64] = {
...
uint32_t
NrBufferSizeLevelBsr::BsrId2BufferSize (uint8_t val)
{
  NS_ABORT_MSG_UNLESS (val < 64, "val = " << val << " is out of range");
  return NrBufferSizeLevelBsrTable[val];
}

uint8_t
NrBufferSizeLevelBsr::BufferSize2BsrId (uint32_t val)
{
  int index = 0;
  if (NrBufferSizeLevelBsrTable[63] < val)
    {
      index = 63;
    }
  else
    {
      while (NrBufferSizeLevelBsrTable[index] < val)
        {
          NS_ASSERT (index < 64);
          index++;
        }
...
uint8_t
NrTransmissionModesLayers::TxMode2LayerNum (uint8_t txMode)
{
  uint8_t nLayer = 0;
  switch (txMode)
    {
...
double 
NrEutranMeasurementMapping::RsrpRange2Dbm (uint8_t range)
{
...
uint8_t 
NrEutranMeasurementMapping::Dbm2RsrpRange (double dbm)
{
  // 3GPP TS 36.133 section 9.1.4 RSRP Measurement Report Mapping
...
double 
NrEutranMeasurementMapping::RsrqRange2Db (uint8_t range)
{
...
uint8_t 
NrEutranMeasurementMapping::Db2RsrqRange (double db)
{
  // 3GPP TS 36.133 section 9.1.7 RSRQ Measurement Report Mapping
...
double 
NrEutranMeasurementMapping::QuantizeRsrp (double v)
{
  return RsrpRange2Dbm (Dbm2RsrpRange (v));
}

double 
NrEutranMeasurementMapping::QuantizeRsrq (double v)
{
  return RsrqRange2Db (Db2RsrqRange (v));
}

double
NrEutranMeasurementMapping::IeValue2ActualHysteresis (uint8_t hysteresisIeValue)
{
...
uint8_t
NrEutranMeasurementMapping::ActualHysteresis2IeValue (double hysteresisDb)
{
...
double
NrEutranMeasurementMapping::IeValue2ActualA3Offset (int8_t a3OffsetIeValue)
{
...
int8_t
NrEutranMeasurementMapping::ActualA3Offset2IeValue (double a3OffsetDb)
{
...
double
NrEutranMeasurementMapping::IeValue2ActualQRxLevMin (int8_t qRxLevMinIeValue)
{
...
double
NrEutranMeasurementMapping::IeValue2ActualQQualMin (int8_t qQualMinIeValue)
{
