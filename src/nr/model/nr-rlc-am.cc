...
NrRlcAm::NrRlcAm ()
{
  ...
  m_ngcX2RlcUser = new NgcX2RlcSpecificUser<NrRlcAm> (this);
  m_ngcX2RlcProvider = 0;

  m_txonQueue = CreateObject<CoDelQueueDisc> ();
  m_txonQueue->Initialize ();

  m_traceBufferSizeEvent = Simulator::Schedule(MilliSeconds(2), &NrRlcAm::BufferSizeTrace, this);
  sumPacketSize=0;
  lastSumPacketSize=0;
  TotalTime=0;
...
NrRlcAm::BufferSizeTrace()
{
  ...
  m_traceBufferSizeEvent = Simulator::Schedule(MilliSeconds(10), &NrRlcAm::BufferSizeTrace, this);
}

std::string
NrRlcAm::GetBufferSizeFilename()
{
  return m_bufferSizeFilename;
}

void
NrRlcAm::SetBufferSizeFilename(std::string filename)
{
...
{
  static TypeId tid = TypeId ("ns3::NrRlcAm")
    .SetParent<NrRlc> ()
    .SetGroupName("Nr")
   ...
   .AddAttribute ("MaxTxBufferSize",
    ...
    .AddAttribute ("EnableAQM",
                  "Enable active queue management (CoDel)",
                   BooleanValue (false),
                   MakeBooleanAccessor (&NrRlcAm::m_enableAqm),
                   MakeBooleanChecker ())
   .AddAttribute ("BufferSizeFilename",
                   ...
                   MakeStringAccessor (&NrRlcAm::SetBufferSizeFilename),
                   MakeStringChecker ())
    ;
  return tid;
}
...
NrRlcAm::DoTransmitPdcpPdu (Ptr<Packet> p)
{
  ...
  if (m_ngcX2RlcProvider !=0 && reporting_Start==0){ //sjkang1114
       ...
       std::cout << "will send assistant info from NrRlcAm" <<std::endl;
       RecordingQueueStatistics();
       Simulator::Schedule(MilliSeconds(10),&NrRlcAm::SendAssistantInformation,this, info);
...
void 
NrRlcAm::DoSendMcPdcpSdu(NgcX2Sap::UeDataParams params)
{
  NS_LOG_FUNCTION(this);
  ...
  if ( m_statusPduRequested && ! m_statusProhibitTimer.IsRunning () )
    {
      ...
      m_statusProhibitTimer = Simulator::Schedule (m_statusProhibitTimerValue,
                                                   &NrRlcAm::ExpireStatusProhibitTimer, this);
      return;
    }
                      ...
                      if (! m_pollRetransmitTimer.IsRunning () )
                        {
                          ...
                          m_pollRetransmitTimer = Simulator::Schedule (m_pollRetransmitTimerValue,
                                                                       &NrRlcAm::ExpirePollRetransmitTimer, this);
                        }
                      else
                        {
...
//                  return;
                  ...
                  // Calculate the Polling Bit (5.2.2.1)
                  firstSegHdr.SetPollingBit (NrRlcAmHeader::STATUS_REPORT_NOT_REQUESTED);
                  ...
                  {
                    m_pollRetransmitTimerJustExpired = false;
                    firstSegHdr.SetPollingBit (NrRlcAmHeader::STATUS_REPORT_IS_REQUESTED);
                    m_pduWithoutPoll = 0;
                    m_byteWithoutPoll = 0;
                  ...
                  // set flags
                  firstSegHdr.SetResegmentationFlag (NrRlcAmHeader::SEGMENT);
                  firstSegHdr.SetLastSegmentFlag (NrRlcAmHeader::NO_LAST_PDU_SEGMENT);
                  //firstSegHdr.PushLengthIndicator (firstPduSegSize);

                  // build header for second segment
                  NrRlcAmHeader nextSegHdr = firstSegHdr;
 ...
 */
bool 
NrRlcAm::IsInsideTransmittingWindow(){
...
void
NrRlcAm::DoNotifyDlHarqDeliveryFailure (uint8_t harqId)
{
  NS_LOG_FUNCTION (this);
...
void
NrRlcAm::DoNotifyUlHarqDeliveryFailure (uint8_t harqId)
{
  // send NACK
}
...
void
NrRlcAm::Reassemble (Ptr<Packet> packet)
{
  NrRlcAmHeader rlcAmHeader;
  packet->RemoveHeader (rlcAmHeader);
      ...
      switch (m_assemblingState)
        {
          ...
          case WAITING_SI_SF:
                  switch (framingInfo)
                    {
                      case (NrRlcAmHeader::NO_FIRST_BYTE | NrRlcAmHeader::LAST_BYTE): //finish the previous S0.
...
// LL HO
void 
NrRlcAm::RlcPdusToRlcSdus (std::vector < NrRlcAm::RetxPdu > RlcPdus){

  NS_LOG_DEBUG (this << "in RlcPdusTo..." );
  uint16_t isGotExpectedSeqNumber = 0;
  for ( std::vector <NrRlcAm::RetxPdu>::iterator it = RlcPdus.begin(); it != RlcPdus.end (); it++)
        {
          if (it->m_pdu == 0){
            continue;
          }
...
//                }
                ...
                if(rlcAmHeader.GetSegmentOffset() == lastSegHdr.GetLastOffset () || rlcAmHeader.GetSegmentOffset() + 32768 == lastSegHdr.GetLastOffset ()) 
                {
                  ...
                  if (rlcAmHeader.GetLastSegmentFlag () == NrRlcAmHeader::LAST_PDU_SEGMENT)
                  {
          ...
          if ( m_vrH > m_vrR )
            {
              ...
              m_reorderingTimer = Simulator::Schedule (m_reorderingTimerValue,
                                                       &NrRlcAm::ExpireReorderingTimer ,this);
              m_vrX = m_vrH;
...
bool
NrRlcAm::IsInsideReceivingWindow (SequenceNumber10 seqNumber)
{
...
void
NrRlcAm::TriggerReceivePdcpPdu(Ptr<Packet> p)
{
  if(!isMc) 
  {
...
void
NrRlcAm::DoReportBufferStatus (void)
{
  NS_LOG_FUNCTION (this);
...
void
NrRlcAm::ExpireReorderingTimer (void)
{
  NS_LOG_FUNCTION (this);
...
void
NrRlcAm::ExpirePollRetransmitTimer (void)
{
  NS_LOG_FUNCTION (this);
...
void 
NrRlcAm::ExpireStatusProhibitTimer (void)
{
  NS_LOG_FUNCTION (this);
}

void
NrRlcAm::ExpireRbsTimer (void)
{
...
void
NrRlcAm::SendAssistantInformation(NgcX2Sap::AssistantInformationForSplitting info){
   ...
   // std::cout<<"MmWave at Rlc Am  " <<(double) (info.Txed_Q_Delay+info.Tx_On_Q_Delay)/10e3<<"s"<<std::endl;

//std::cout << "will send NR assist info" <<m_rlcSapUser<<std::endl;

	      Simulator::Schedule(MilliSeconds(1),&NrRlcAm::SendAssistantInformation,this, info);
}

void
NrRlcAm::SendNrAssistantInformation(NgcX2Sap::AssistantInformationForSplitting info){//sjkang1114
	info.targetCellId = m_ueDataParams.targetCellId;
	info.sourceCellId = m_ueDataParams.sourceCellId;
    info.Tx_On_Q_Size= m_txonBufferSize + m_txedBufferSize;
    info.Re_TX_Q_Size= m_retxBufferSize;
    ...
    info.rnti = m_rnti;
    info.drbId = m_drbId;
//std::cout<<"NR at Rlc Am  " <<(double) (info.Txed_Q_Delay+info.Tx_On_Q_Delay)/10e3 <<"s"<<std::endl;
  m_rlcSapUser->SendNrAssi(info);

	//}
	      Simulator::Schedule(MilliSeconds(1),&NrRlcAm::SendNrAssistantInformation,this, info);
}

void
NrRlcAm::CalculatePathThroughput (std::ofstream *stream) // woody
{
...
//std::cout << sumPacketSize << std::endl;
  *stream << now.GetSeconds () << "\t" << cur << "\t"<<(double)((TotalPackets*8)/TotalTime)/1e6 <<std::endl;
  ...
  Simulator::Schedule (MilliSeconds (100), &NrRlcAm::CalculatePathThroughput, this, stream);
}
void
NrRlcAm::RecordingQueueStatistics(){//sjkang1116
...
Simulator::Schedule(MilliSeconds(1.0),&NrRlcAm::RecordingQueueStatistics,this);
}
void
NrRlcAm::DoRequestAssistantInfo(){

	             NgcX2Sap::AssistantInformationForSplitting info;
	            std::cout << "will send NR assistant info from NrRlcAm" <<std::endl;
	            RecordingQueueStatistics();
	            Simulator::Schedule(MilliSeconds(10),&NrRlcAm::SendNrAssistantInformation,this, info);
