...
void
NrInterference::StartRx (Ptr<const SpectrumValue> rxPsd)
{ 
  ...
  if (m_receiving == false)
    {
      ...
      m_receiving = true;
      for (std::list<Ptr<NrChunkProcessor> >::const_iterator it = m_rsPowerChunkProcessorList.begin (); it != m_rsPowerChunkProcessorList.end (); ++it)
        {
          (*it)->Start ();
        }
      for (std::list<Ptr<NrChunkProcessor> >::const_iterator it = m_interfChunkProcessorList.begin (); it != m_interfChunkProcessorList.end (); ++it)
        {
          (*it)->Start ();
        }
      for (std::list<Ptr<NrChunkProcessor> >::const_iterator it = m_sinrChunkProcessorList.begin (); it != m_sinrChunkProcessorList.end (); ++it)
        {
          (*it)->Start (); 
        }
    }
...
void
NrInterference::AddSignal (Ptr<const SpectrumValue> spd, const Time duration)
{
  ...
  Simulator::Schedule (duration, &NrInterference::DoSubtractSignal, this, spd, signalId);
}


void
NrInterference::DoAddSignal  (Ptr<const SpectrumValue> spd)
{ 
...
void
NrInterference::DoSubtractSignal  (Ptr<const SpectrumValue> spd, uint32_t signalId)
{ 
  NS_LOG_FUNCTION (this << *spd);
  ConditionallyEvaluateChunk ();   
...
void
NrInterference::SetNoisePowerSpectralDensity (Ptr<const SpectrumValue> noisePsd)
{
...
void
NrInterference::AddRsPowerChunkProcessor (Ptr<NrChunkProcessor> p)
{
...
void
NrInterference::AddSinrChunkProcessor (Ptr<NrChunkProcessor> p)
{
  NS_LOG_FUNCTION (this << p);
...
void
NrInterference::AddInterferenceChunkProcessor (Ptr<NrChunkProcessor> p)
{
  NS_LOG_FUNCTION (this << p);
