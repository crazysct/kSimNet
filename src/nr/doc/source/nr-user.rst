...
The ns-3 NR model is a software library that allows the simulation of
NR networks, optionally including the Evolved Packet Core (NGC).  The
...
following steps:
 ...
 2. *Write a simulation program* that recreates the desired scenario
    topology/architecture. This is done accessing the ns-3 NR model
    library using the ``ns3::NrHelper`` API defined in ``src/nr/helper/nr-helper.h``. 
...
Here is the minimal simulation program that is needed to do an NR-only simulation (without NGC).
...
#. Create ``Node`` objects for the eNB(s) and the UEs::
   ...
   an NR protocol stack installed; they're just empty nodes. 
...
#. Install an NR protocol stack on the eNB(s)::

      NetDeviceContainer enbDevs;
      enbDevs = nrHelper->InstallEnbDevice (enbNodes);

#. Install an NR protocol stack on the UEs::
...
Configuration of NR model parameters
-------------------------------------

All the relevant NR model parameters are managed through the ns-3
...
specifying the new default values that you want to use for some attributes::
   ...
   default ns3::NrEnbNetDevice::DlEarfcn "100"
   default ns3::NrEnbNetDevice::UlEarfcn "18100"
   default ns3::NrUePhy::TxPower "10"
   default ns3::NrUePhy::NoiseFigure "9"
   default ns3::NrEnbPhy::TxPower "30"
   default ns3::NrEnbPhy::NoiseFigure "5"

Supposing your simulation program is called
``src/nr/examples/nr-sim-with-input``, you can now pass these
settings to the simulation program in the following way::

   ./waf --command-template="%s --ns3::ConfigStore::Filename=input-defaults.txt --ns3::ConfigStore::Mode=Load --ns3::ConfigStore::FileFormat=RawText" --run src/nr/examples/nr-sim-with-input


Furthermore, you can generate a template input file with the following
command::

   ./waf --command-template="%s --ns3::ConfigStore::Filename=input-defaults.txt --ns3::ConfigStore::Mode=Save --ns3::ConfigStore::FileFormat=RawText" --run src/nr/examples/nr-sim-with-input
...
simulator, including lots of non-NR attributes.

Configure NR MAC Scheduler
---------------------------
 ...
 * TBFQ scheduler::
  ...
  Ptr<NrHelper> nrHelper = CreateObject<NrHelper> ();
  nrHelper->SetSchedulerAttribute("DebtLimit", IntegerValue(yourvalue)); // default value -625000 bytes (-5Mb)
  nrHelper->SetSchedulerAttribute("CreditLimit", UintegerValue(yourvalue)); // default value 625000 bytes (5Mb)
  nrHelper->SetSchedulerAttribute("TokenPoolSize", UintegerValue(yourvalue)); // default value 1 byte
  nrHelper->SetSchedulerAttribute("CreditableThreshold", UintegerValue(yourvalue)); // default value 0

 * PSS scheduler::

  Ptr<NrHelper> nrHelper = CreateObject<NrHelper> ();
  nrHelper->SetSchedulerAttribute("nMux", UIntegerValue(yourvalue)); // the maximum number of UE selected by TD scheduler
  nrHelper->SetSchedulerAttribute("PssFdSchedulerType", StringValue("CoItA")); // PF scheduler type in PSS
...
Maximum Bit Rate (MBR) in ngc bearer QoS parameters. Users can use following codes to define GBR and MBR in both downlink and uplink::
  ...
  nrHelper->ActivateDedicatedEpsBearer (ueDevs, bearer, NgcTft::Default ());
...
Many useful attributes of the NR-NGC model will be described in the
...
program, like this::
     ...
     ./waf --run lena-simple --command-template="%s --PrintAttributes=ns3::NrHelper"


You can try also with other NR and NGC objects, like this::
   
     ./waf --run lena-simple --command-template="%s --PrintAttributes=ns3::NrEnbNetDevice"
     ./waf --run lena-simple --command-template="%s --PrintAttributes=ns3::NrEnbMac"
     ./waf --run lena-simple --command-template="%s --PrintAttributes=ns3::NrEnbPhy"
     ./waf --run lena-simple --command-template="%s --PrintAttributes=ns3::NrUePhy"
...
The ns-3 NR model currently supports the output to file of PHY, MAC, RLC
...
It has to be noted that the ns-3 NR module is able to work with any fading trace file that complies with the above described ASCII format. Hence, other external tools can be used to generate custom fading traces, such as for example other simulators or experimental devices.
...
And for setting the parameters::
  ...
  nrHelper->SetFadingModelAttribute ("TraceFilename", StringValue ("src/nr/model/fading-traces/fading_trace_EPA_3kmph.fad"));
  nrHelper->SetFadingModelAttribute ("TraceLength", TimeValue (Seconds (10.0)));
  nrHelper->SetFadingModelAttribute ("SamplesNum", UintegerValue (10000));
  nrHelper->SetFadingModelAttribute ("WindowSize", TimeValue (Seconds (0.5)));
  nrHelper->SetFadingModelAttribute ("RbNum", UintegerValue (100));

It has to be noted that, ``TraceFilename`` does not have a default value, therefore is has to be always set explicitly.

The simulator provide natively three fading traces generated according to the configurations defined in in Annex B.2 of [TS36104]_. These traces are available in the folder ``src/nr/model/fading-traces/``). An excerpt from these traces is represented in the following figures.
...
We now explain by examples how to use the buildings model (in particular, the ``MobilityBuildingInfo`` and the ``BuildingPropagationModel`` classes) in an ns-3 simulation program to setup an NR simulation scenario that includes buildings and indoor nodes.
...
The selection of the working frequency of the propagation model has to be done with the standard ns-3 attribute system as described in the correspond section ("Configuration of NR model parameters") by means of the DlEarfcn and UlEarfcn parameters, for instance::

   nrHelper->SetEnbDeviceAttribute ("DlEarfcn", UintegerValue (100));
   nrHelper->SetEnbDeviceAttribute ("UlEarfcn", UintegerValue (18100));
...
Is this subsection we illustrate how to configure the MIMO parameters. NR defines 7 types of transmission modes:
...
According to model implemented, the simulator includes the first three transmission modes types. The default one is the Transmission Mode 1 (SISO). In order to change the default Transmission Mode to be used, the attribute ``DefaultTransmissionMode`` of the ``NrEnbRrc`` can be used, as shown in the following::

  Config::SetDefault ("ns3::NrEnbRrc::DefaultTransmissionMode", UintegerValue (0)); // SISO
  Config::SetDefault ("ns3::NrEnbRrc::DefaultTransmissionMode", UintegerValue (1)); // MIMO Tx diversity (1 layer)
  Config::SetDefault ("ns3::NrEnbRrc::DefaultTransmissionMode", UintegerValue (2)); // MIMO Spatial Multiplexity (2 layers)
...
``NrHelper`` instance right before the creation of the
``EnbNetDevice``, as shown in the following::
  
  nrHelper->SetEnbAntennaModelType ("ns3::CosineAntennaModel");
  nrHelper->SetEnbAntennaModelAttribute ("Orientation", DoubleValue (0));
  nrHelper->SetEnbAntennaModelAttribute ("Beamwidth",   DoubleValue (60);
...
As an example, here is the REM that can be obtained with the example program lena-dual-stripe, which shows a three-sector NR macrocell in a co-channel deployment with some residential femtocells randomly deployed in two blocks of apartments.
...
While, the solution based on the physical error model can be controlled with::
  ...
  Config::SetDefault ("ns3::NrAmc::AmcModel", EnumValue (NrAmc::MiErrorModel));
...
simulate the NGC in addition to the NR radio access network. The use
of NGC allows to use IPv4 networking with NR devices. In other words,
you will be able to use the regular ns-3 applications and sockets over
IPv4 over NR, and also to connect an NR network to any other IPv4
network you might have in your simulation.

First of all, in addition to ``NrHelper`` that we already introduced
...
Then, you need to tell the NR helper that the NGC will be used::

  nrHelper->SetNgcHelper (ngcHelper);

the above step is necessary so that the NR helper will trigger the
...
automatically take care of the necessary setup, such as N2 link
creation and N2 bearer setup. All this will be done without the
intervention of the user.

Calling ``nrHelper->SetNgcHelper (ngcHelper)`` enables the use of
NGC, and has the side effect that any new ``NrEnbRrc`` that is
...
traffic from/to the NR radio access network.  Still,
...
It's important to specify routes so that the remote host can reach NR
UEs. One way of doing this is by exploiting the fact that the
``PointToPointNgcHelper`` will by default assign to NR UEs an IP address in the
...
Now, you should go on and create NR eNBs and UEs as explained in the
previous sections. You can of course configure other NR aspects such
...
to configure an NR-only simulation, you would then normally do
something like this::

      NetDeviceContainer ueNrDevs = nrHelper->InstallUeDevice (ueNodes);
      nrHelper->Attach (ueNrDevs, enbNrDevs.Get (0));        
      ...
      for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
        {
          ...
          Ipv4InterfaceContainer ueIpIface = ngcHelper->AssignUeIpv4Address (NetDeviceContainer (ueNrDevice));
...
for an application at the UE communicating on port 1234::
      ...
      tft->Add (pf);  
      nrHelper->ActivateDedicatedEpsBearer (ueNrDevs, EpsBearer (EpsBearer::NGBR_VIDEO_TCP_DEFAULT), tft);
...
Finally, you can install applications on the NR UE nodes that communicate
...
UdpClient application on the remote host, and a PacketSink on the NR UE
...
In the previous section we used PointToPoint links for the connection between the eNBs and the SMF (N2-U interface) and among eNBs (X2-U and X2-C interfaces). The NR module supports using emulated links instead of PointToPoint links. This is achieved by just replacing the creation of ``NrHelper`` and ``NgcHelper`` with the following code::
...
The attributes ``ns3::EmuNgcHelper::smfDeviceName`` and ``ns3::EmuNgcHelper::enbDeviceName`` are used to set the name of the devices used for transporting the N2-U, X2-U and X2-C interfaces at the SMF and eNB, respectively. We will now show how this is done in an example where we execute the example program ``lena-simple-ngc-emu`` using two virtual ethernet interfaces. 

First of all we build ns-3 appropriately::

  # configure
  ./waf configure --enable-sudo --enable-modules=nr,fd-net-device --enable-examples 
...
linked modules::
  ...
  ./waf configure -d optimized --enable-static --enable-modules=nr --enable-examples --enable-sudo
...
run an emulated NR-NGC network over a real multi-layer packet-optical
transport network.
...
attaching a UE to an eNodeB is done by calling ``NrHelper::Attach`` function.
...
This method uses the ``NrHelper::Attach`` function mentioned above. It has been
the only available network attachment method in earlier versions of NR module.
It is typically invoked before the simulation begins::

   nrHelper->Attach (ueDevs, enbDev); // attach one or more UEs to a single eNodeB

``NrHelper::InstallEnbDevice`` and ``NrHelper::InstallUeDevice`` functions
...
this is not the case in this ``NrHelper::Attach`` function. The other network
...
version of the ``NrHelper::Attach`` function, as shown below::

   nrHelper->Attach (ueDevs); // attach one or more UEs to a strongest cell
...
NR-only simulations must resort to manual attachment method.
...
example using the following ``NrHelper`` functions::
   ...
   NetDeviceContainer csgEnbDevs = nrHelper->InstallEnbDevice (csgEnbNodes);
   NetDeviceContainer csgUeDevs = nrHelper->InstallUeDevice (csgUeNodes);

Then enable the initial cell selection procedure on the UEs:: 

   nrHelper->Attach (csgUeDevs);
...
new ``NrRrcSap::ReportConfigEutra`` instance and pass it to the
``NrEnbRrc::AddUeMeasReportConfig`` function. The function will return the
...
listening to the existing ``NrEnbRrc::RecvMeasurementReport`` trace source.
...
within the container ``devs``::
   ...
   config.reportInterval = NrRrcSap::ReportConfigEutra::MS480;
   ...
   for (it = devs.Begin (); it != devs.End (); it++)
   {
     ...
     Ptr<NrEnbNetDevice> enbDev = dev->GetObject<NrEnbNetDevice> ();
     Ptr<NrEnbRrc> enbRrc = enbDev->GetRrc ();
...
due to Section 9.1.4 and 9.1.7 of [TS36133]_. The ``NrEutranMeasurementMapping``
...
the ``measId`` field, contained within the ``NrRrcSap::MeasurementReport``
...
``NrRrcSap::ReportConfigEutra``) of the UE measurements parameter is open for
...
attribute ``NrEnbRrc::AdmitHandoverRequest`` to `false`. As an example, you can
run the ``lena-x2-handover`` program and setting the attribute in this way::

   NS_LOG=NgcX2:NrEnbRrc ./waf --run lena-x2-handover --command="%s --ns3::NrEnbRrc::AdmitHandoverRequest=false"
...
scheduling an explicit handover event. The ``NrHelper`` object provides a
convenient method for the scheduling of a handover event. As an example, let us
assume that ``ueNrDevs`` is a ``NetDeviceContainer`` that contains the UE that
is to be handed over, and that ``enbNrDevs`` is another ``NetDeviceContainer``
that contains the source and the target eNB. Then, a handover at 0.1s can be
scheduled like this::

   nrHelper->HandoverRequest (Seconds (0.100), 
                               ueNrDevs.Get (0), 
                               enbNrDevs.Get (0), 
                               enbNrDevs.Get (1));
...
Selecting a handover algorithm is done via the ``NrHelper`` object and its
...
Finally, the ``InstallEnbDevice`` function of ``NrHelper`` will instantiate one
...
eNodeB and the target eNodeB) and the N2 interface (between the target eNodeB
...
The RRC model, in particular the ``NrEnbRrc`` and ``NrUeRrc``
...
The example program ``src/nr/examples/lena-x2-handover.cc``
...
information, you can run the program like this::
    ...
    NS_LOG=NrEnbRrc:NrUeRrc:NgcX2 ./waf --run lena-x2-handover
...
(i.e. NrFrNoOpAlgorithm) is installed in eNb. It acts as if FR 
...
Selecting a FR algorithm is done via the ``NrHelper`` object and 
...
configure required values for ``NrEnbNetDevice``::
...
``ns3::NrFrHardAlgorithm`` uses one sub-band. To configure this sub-band user need 
...
Example configuration of NrFrHardAlgorithm can be done in following way::

   nrHelper->SetFfrAlgorithmType ("ns3::NrFrHardAlgorithm");
   nrHelper->SetFfrAlgorithmAttribute ("DlSubBandOffset", UintegerValue (8));
   nrHelper->SetFfrAlgorithmAttribute ("DlSubBandwidth", UintegerValue (8));
   nrHelper->SetFfrAlgorithmAttribute ("UlSubBandOffset", UintegerValue (8));
   nrHelper->SetFfrAlgorithmAttribute ("UlSubBandwidth", UintegerValue (8));
   NetDeviceContainer enbDevs = nrHelper->InstallEnbDevice (enbNodes.Get(0));
...
``NrEnbPhy::TxPower - 3dB``, power in edge area equals ``NrEnbPhy::TxPower + 3dB``::
   ...
   nrHelper->SetFfrAlgorithmAttribute ("RsrqThreshold", UintegerValue (20));
   ...
   nrHelper->SetFfrAlgorithmAttribute ("EdgeAreaTpc", UintegerValue (2));
   NetDeviceContainer enbDevs = nrHelper->InstallEnbDevice (enbNodes.Get(0));
...
equals ``NrEnbPhy::TxPower``, power in edge area equals ``NrEnbPhy::TxPower + 3dB``::
...
Power in center area equals ``NrEnbPhy::TxPower - 3dB``, power in medium area equals 
``NrEnbPhy::TxPower + 3dB``, power in edge area equals ``NrEnbPhy::TxPower + 3dB``::
...
thresholds are set to value of 10. Power in center area equals ``NrEnbPhy::TxPower - 6dB``, 
power in edge area equals ``NrEnbPhy::TxPower + 0dB``::
...
Power in center area equals ``NrEnbPhy::TxPower - 0dB``, power in edge area equals ``NrEnbPhy::TxPower + 3dB``::

  nrHelper->SetFfrAlgorithmType("ns3::NrFfrDistributedAlgorithm");
  nrHelper->SetFfrAlgorithmAttribute("CalculationInterval", TimeValue(MilliSeconds(500)));
  nrHelper->SetFfrAlgorithmAttribute ("RsrqThreshold", UintegerValue (25));
...
the boolean attribute ``ns3::NrUePhy::EnableUplinkPowerControl`` to true.

User can switch between Open Loop Power Control and Closed Loop Power Control mechanisms
by setting the boolean attribute ``ns3::NrUePowerControl::ClosedLoop``. 
...
Example configuration is presented below::
  ...
  Config::SetDefault ("ns3::NrEnbPhy::TxPower", DoubleValue (30));
...
The directory ``src/nr/examples/`` contains some example simulation programs that
show how to simulate different NR scenarios. 


Reference scenarios
-------------------

There is a vast amount of reference NR simulation scenarios which can
 ...
 * The dual stripe model [R4-092042]_, which is partially implemented in the
   example program ``src/nr/examples/lena-dual-stripe.cc``. This example
...
campaign using the NR module of |ns3|. The objective of the campaign is to
compare the effect of each built-in handover algorithm of the NR module.
 ...
 * We can add a ``--generateRem=1`` argument to generate the files necessary for
   ...
   Figure :ref:`fig-nr-handover-campaign-rem` below, which can be produced by
...
.. _fig-nr-handover-campaign-rem:

.. figure:: figures/nr-handover-campaign-rem.*
   :align: center
...
FR algorithm by overriding the default attribute ``ns3::NrHelper::FfrAlgorithm``.
Example command to run ``lena-frequency-reuse`` with Soft FR algorithm is presented below::

   $ ./waf --run "lena-frequency-reuse --ns3::NrHelper::FfrAlgorithm=ns3::NrFrSoftAlgorithm"
...
Radio Environment Map for Soft FR is presented in Figure :ref:`fig-nr-soft-fr-1-rem`.

.. _fig-nr-soft-fr-1-rem:

.. figure:: figures/nr-fr-soft-1-rem.*
   :align: center
...
inside script)::
   ...
   $ ./waf --run "lena-frequency-reuse --ns3::NrHelper::FfrAlgorithm=ns3::NrFfrSoftAlgorithm
     --generateSpectrumTrace=true"

Example spectrum analyzer trace is presented in figure :ref:`fig-nr-soft-ffr-2-spectrum-trace`.
...
.. _fig-nr-soft-ffr-2-spectrum-trace:

.. figure:: figures/nr-ffr-soft-2-spectrum-trace.*
   :align: center
...
User needs to specify FR algorithm by overriding the default attribute ``ns3::NrHelper::FfrAlgorithm``.
