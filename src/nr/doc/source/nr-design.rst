...
An overview of the  NR-NGC simulation model is depicted in
the figure :ref:`fig-ngc-topology`. There are two main components:

 * the NR Model. This model includes the NR Radio Protocol
 ...
 * the NGC Model. This models includes core network
   ...
   reside within the SMF, UPF and AMF nodes, and partially within the
   eNB nodes.
...
.. figure:: figures/ngc-topology.*
   ...
   Overview of the NR-NGC simulation model
...
The NR model has been designed to support the evaluation of the following aspects of NR systems:  
...
In order to model NR systems to a level of detail that is sufficient to allow a
...
have been considered:
 ...
 #. To be more representative of the NR standard, as well as to be as
    ...
    simulator, we make it possible for NR equipment vendors and
    operators to test in a simulative environment exactly the same
    algorithms that would be deployed in a real system.  
 #. The NR simulation model should contain its own implementation of
 ...
 #. The model is to be used to simulate the transmission of IP packets
    ...
    that in NR the Scheduling and Radio Resource Management do not
...
simulation of end-to-end IP connectivity over the NR model. 
...
The architecture of the NR radio protocol stack model of the UE is
represented in the figures :ref:`fig-nr-arch-ue-data` and
:ref:`fig-nr-arch-ue-ctrl` which highlight respectively the data
plane and the control plane.


.. _fig-nr-arch-ue-data:
 
.. figure:: figures/nr-arch-ue-data.*
   :align: center

   NR radio protocol stack architecture for the UE on the data plane


.. _fig-nr-arch-ue-ctrl:

.. figure:: figures/nr-arch-ue-ctrl.*
   :align: center

   NR radio protocol stack architecture for the UE on the control plane


The architecture of the PHY/channel model of the UE is represented in figure :ref:`fig-nr-ue-phy`. 


.. _fig-nr-ue-phy:
   
.. figure:: figures/nr-ue-phy.*
   :align: center
...
The architecture of the NR radio protocol stack model of the eNB is
represented in the figures :ref:`fig-nr-arch-enb-data` and
:ref:`fig-nr-arch-enb-ctrl` which highlight respectively the data plane
and the control plane. 


.. _fig-nr-arch-enb-data:
   
.. figure:: figures/nr-arch-enb-data.*
   :align: center

   NR radio protocol stack architecture for the eNB on the data plane


.. _fig-nr-arch-enb-ctrl:
   
.. figure:: figures/nr-arch-enb-ctrl.*
   :align: center

   NR radio protocol stack architecture for the eNB on the control plane


The architecture of the PHY/channel model of the eNB is represented in figure :ref:`fig-nr-enb-phy`. 


.. _fig-nr-enb-phy:
   
.. figure:: figures/nr-enb-phy.*
   :align: center
...
In Figure :ref:`fig-nr-ngc-e2e-data-protocol-stack`, we represent the
end-to-end NR-NGC data plane protocol stack as it is modeled in the
...
specified by 3GPP. On the other hand, for both the N2-U protocol stack and
the NR radio protocol stack all the protocol layers specified by 3GPP
are present. 


.. _fig-nr-ngc-e2e-data-protocol-stack:
   
.. figure:: figures/nr-ngc-e2e-data-protocol-stack.*
   :align: center

   NR-NGC data plane protocol stack
...
modeled explicitly are the N2-AP, the X2-AP and the N11 interfaces. 

We note that the N2-AP and the N11 interfaces are modeled in a simplified
...
and the AMF for the N2-AP interface, and the AMF and the SMF for the
N11 interface). In practice, this means that the primitives of these
...
For channel modeling purposes, the NR module uses the ``SpectrumChannel``
...
NR module requires the use of the ``MultiModelSpectrumChannel`` in
...
used within the NR module.  



Use of the Buildings model with NR
+++++++++++++++++++++++++++++++++++

The recommended propagation model to be used with the NR
module is the one provided by the Buildings module, which was in fact
designed specifically with NR (though it can be used with other
...
The NR module considers FDD only, and implements downlink and uplink propagation separately. As a consequence, the following pathloss computations are performed

  * MBS <-> UE (indoor and outdoor)
  * SC (indoor and outdoor) <-> UE (indoor and outdoor)
 
The NR model does not provide the following pathloss computations:
...
The simulator provides a matlab script (``src/nr/model/fading-traces/fading-trace-generator.m``) for generating traces based on the format used by the simulator. 
 ...
 * time granularity: we need one fading value per TTI, i.e., every 1 ms (as this is the granularity in time of the ns-3 NR PHY model).
 * frequency granularity: we need one fading value per RB (which is the frequency granularity of the spectrum model used by the ns-3 NR model).
...
Being based on the ``SpectrumPhy``, the NR PHY model supports antenna
...
The physical layer model provided in this NR simulator is based on
...
The subframe is divided into control and data part as described in Figure :ref:`fig-nr-subframe-structure`.

.. _fig-nr-subframe-structure:

.. figure:: figures/nr-subframe-structure.*
   :align: center
   :width: 300px

   NR subframe division.
...
To switch between this two CQI generation approaches, ``NrHelper::UsePdschForCqiGeneration`` needs to be configured: false for first approach and true for second approach (true is default value)::
...
The sequence diagram of Figure :ref:`fig-nr-phy-interference` shows how interfering signals are processed to calculate the SINR, and how SINR is then used for the generation of CQI feedback.


.. _fig-nr-phy-interference:
   
.. figure:: figures/nr-phy-interference.*
   :align: center
...
The usage of the radio spectrum by eNBs and UEs in NR is described in
[TS36101]_. In the simulator, radio spectrum usage is modeled as follows. 
Let :math:`f_c` denote the  NR Absolute Radio Frequency Channel Number, which
...
To do this the Vienna NR Simulator [ViennaNrSim]_ has been used for what concerns the extraction of link layer performance and the Mutual Information Based Effective SINR (MIESM) as LSM mapping function using part of the work recently published by the Signet Group of University of Padua [PaduaPEM]_.
...
Integration of the BLER curves in the ns-3 NR module
-----------------------------------------------------

The model implemented uses the curves for the LSM of the recently NR PHY Error Model released in the ns3 community by the Signet Group [PaduaPEM]_ and the new ones generated for different CB sizes. The ``NrSpectrumPhy`` class is in charge of evaluating the TB BLER thanks to the methods provided by the ``NrMiErrorModel`` class, which is in charge of evaluating the TB BLER according to the vector of the perceived SINR per RB, the MCS and the size in order to proper model the segmentation of the TB in CBs. In order to obtain the vector of the perceived SINR two instances of ``NrPemSinrChunkProcessor`` (child of ``NrChunkProcessor`` dedicated to evaluate the SINR for obtaining physical error performance) have been attached to UE downlink and eNB uplink ``NrSpectrumPhy`` modules for evaluating the error model distribution respectively of PDSCH (UE side) and ULSCH (eNB side).

The model can be disabled for working with a zero-losses channel by setting the ``PemEnabled`` attribute of the ``NrSpectrumPhy`` class (by default is active). This can be done according to the standard ns3 attribute system procedure, that is::
...
where :math:`P(k,m)` represents the signal power of the RE :math:`m` within the RB :math:`k`, which, as observed before, is constant within the same RB and equal to :math:`P(k)`, :math:`M` is the number of REs carrying the RS in a RB and :math:`K` is the number of RBs. It is to be noted that :math:`P(k)`, and in general all the powers defined in this section, is obtained in the simulator from the PSD of the RB (which is provided by  the ``NrInterferencePowerChunkProcessor``), in detail:
...
Considering the constraints of the PHY reception chain implementation, and in order to maintain the level of computational complexity low, only RSRP can be directly obtained for all the cells. This is due to the fact that ``NrSpectrumPhy`` is designed for evaluating the interference only respect to the signal of the serving eNB. This implies that the PHY layer is optimized for managing the power signals information with the serving eNB as a reference. However, RSRP and RSRQ of neighbor cell :math:`i` can be extracted by the current information available of the serving cell :math:`j` as detailed in the following:
...
The PHY Error Model model (i.e., the ``NrMiErrorModel`` class already presented before) has been extended for considering IR HARQ according to [wimaxEmd]_, where the parameters for the AWGN curves mapping for MIESM mapping in case of retransmissions are given by:
...
.. figure:: figures/nr-harq-processes-scheme.*
   :align: center

   HARQ processes behavior in NR



The part of HARQ devoted to manage the decodification of the HARQ blocks has been implemented in the ``NrHarqPhy`` and ``NrSpectrumPhy`` classes. The former is in charge of maintaining the HARQ information for each active process . The latter interacts with ``NrMiErrorModel`` class for evaluating the correctness of the blocks received and includes the messaging algorithm in charge of communicating to the HARQ entity in the scheduler the result of the decodifications. These messages are encapsulated in the ``dlInfoListElement`` for DL and ``ulInfoListElement`` for UL and sent through the PUCCH and the PHICH respectively with an ideal error free model according to the assumptions in their implementation. A sketch of the iteration between HARQ and NR protocol stack in represented in Figure:ref:`fig-harq-architecture`.
...
.. figure:: figures/nr-harq-architecture.*
   :align: center

   Interaction between HARQ and NR protocol stack
...
We now briefly describe how resource allocation is handled in NR,
...
layer is performed using a dedicated packet tag (NrRadioBearerTag), which
...
This section describes the ns-3 specific version of the NR MAC
...
implemented within the eNB MAC, i.e., in the file ``nr-enb-mac.cc``.
...
part of our NR simulation module is provided in the following subsections.
...
NR in NS-3 has two versions of TBFQ scheduler: frequency domain TBFQ (FD-TBFQ) and time domain TBFQ (TD-TBFQ). 
...
The Channel and QoS Aware (CQA) Scheduler [Bbojovic2014]_ is an NR
...
each of the messages and signals described in the specs [TS36321]_.
   ...
   - **Random Access (RA) preamble**: in real NR systems this
   ...
   - **Random Access Response (RAR)**: in real NR systems, this is a
     ...
     NrEnbMac will request to the scheduler the allocation of
   ...
   - **Message 3**:  in real NR systems, this is an RLC TM
   ...
   - **Contention Resolution (CR)**: in real NR system, the CR phase
...
Figure :ref:`fig-nr-rlc-implementation-model` shows the
...
.. _fig-nr-rlc-implementation-model:

.. figure:: figures/nr-rlc-implementation-model.*
   :width: 800px
...
Figure :ref:`fig-nr-rlc-data-txon-dl` shows how the upper layers send
data PDUs and how the data flow is processed by the different
entities/services of the NR protocol stack. 

.. _fig-nr-rlc-data-txon-dl:
   
.. figure:: figures/nr-rlc-data-txon-dl.*
   :width: 800px
...
The sequence diagram of Figure :ref:`fig-nr-rlc-data-retx-dl` shows
...
.. _fig-nr-rlc-data-retx-dl:
   
.. figure:: figures/nr-rlc-data-retx-dl.*
   :width: 500px
...
The sequence diagram of Figure :ref:`fig-nr-rlc-data-txon-ul` shows
...
.. _fig-nr-rlc-data-txon-ul:
   
.. figure:: figures/nr-rlc-data-txon-ul.*
   :width: 800px
...
The sequence diagram of Figure :ref:`fig-nr-rlc-data-retx-ul` shows
...
.. _fig-nr-rlc-data-retx-ul:

.. figure:: figures/nr-rlc-data-retx-ul.*
   :width: 500px
...
NR Radio model is used, without the NGC and hence without any IP
...
The RRC model is divided into the following components:
 ...
 - the RRC entities `NrUeRrc` and `NrEnbRrc`, which implement the state
   machines of the RRC entities respectively at the UE and the eNB;
 - the RRC SAPs `NrUeRrcSapProvider`, `NrUeRrcSapUser`,
   `NrEnbRrcSapProvider`, `NrEnbRrcSapUser`, which allow the RRC
   entities to send and receive RRC messages and information
   elmenents;
 - the RRC protocol classes `NrUeRrcProtocolIdeal`,
   `NrEnbRrcProtocolIdeal`, `NrUeRrcProtocolReal`,
   `NrEnbRrcProtocolReal`, which implement two different models for
...
the SAPs that are used is provided in the figures :ref:`fig-nr-arch-ue-data`,
:ref:`fig-nr-arch-ue-ctrl`, :ref:`fig-nr-arch-enb-data` and
:ref:`fig-nr-arch-enb-ctrl`.
...
.. figure:: figures/nr-ue-rrc-states.*
   :scale: 70 %
   :align: center
...
to any of the IDLE states. This choice is done for the following reasons:
 ...
 - as discussed in the section :ref:`sec-design-criteria`, the focus
   of the NR-NGC simulation model is on CONNECTED mode
...
represented in Figure :ref:`fig-nr-enb-rrc-states`.

.. _fig-nr-enb-rrc-states:

.. figure:: figures/nr-enb-rrc-states.*
   :scale: 70 %
   :align: center
...
:ref:`fig-nr-cell-selection-timeline` below. The time diagram on the left side
...
.. _fig-nr-cell-selection-timeline:

.. figure:: figures/nr-cell-selection-timeline.*
   :scale: 80 %
   :align: center
...
simulations. NR-only simulations must use the manual attachment method. See
...
a consequence, the ``NrUePhy::RsrqUeMeasThreshold`` attribute does not apply
during cell search.
...
information blocks are:
 ...
 - System Information Block Type 2 (SIB2)
      ...
      (configurable through `NrEnbRrc::SystemInformationPeriodicity` attribute.
...
``ns3::NrEnbRrc::AdmitRrcConnectionRequest``. There is currently no Radio Admission
...
following simplifying assumptions:
 ...
 - since carrier aggregation is not supported in by the NR module, the
...
parts:
 ...
 #. Measurement configuration (handled by ``NrUeRrc::ApplyMeasConfig``)
 
 #. Performing measurements (handled by ``NrUeRrc::DoReportUeMeasurements``)
 
 #. Measurement report triggering (handled by
    ``NrUeRrc::MeasurementReportTriggering``)
   
 #. Measurement reporting (handled by ``NrUeRrc::SendMeasurementReport``)
...
``NrRrcSap::ReportConfigEutra``, therefore producing the same behaviour as
...
There are two ways to trigger the handover procedure:
 ...
 - *explicitly* (or manually) triggered by the simulation program by scheduling
   an execution of the method ``NrEnbRrc::SendHandoverRequest``; or
...
Handover in 3GPP NR has the following properties:
...
``NrHandoverAlgorithm`` abstract superclass and implementing each of the above
...
provided by the NR module: no-op, A2-A4-RSRQ, and strongest cell handover
...
Figure :ref:`fig-nr-legacy-handover-algorithm` below summarizes this procedure.

.. _fig-nr-legacy-handover-algorithm:

.. figure:: figures/nr-legacy-handover-algorithm.*
   :scale: 70 %
   :align: center
...
:ref:`fig-nr-strongest-cell-handover-algorithm` below, which is taken from the
...
.. _fig-nr-strongest-cell-handover-algorithm:

.. figure:: figures/nr-strongest-cell-handover-algorithm.*
   :align: center
...
NR module supports a simplified *Automatic Neighbour Relation* (ANR) function.
This is handled by the ``NrAnr`` class, which interacts with an eNodeB RRC
...
``NrAnr`` class. Note that the A2-A4-RSRQ handover algorithm also utilizes a
...
Please refer to the corresponding API documentation for ``NrAnrSap`` class for
...
disabled by setting the ``AnrEnabled`` attribute in ``NrHelper`` class to
`false`.
...
NR system it differs slightly from the one used in RRC connection
...
`NrEnbRrcProtocolIdeal`, all RRC messages and information elements
...
This model is implemented in the classes `NrUeRrcProtocolReal` and
`NrEnbRrcProtocolReal` and aims at modeling the transmission of RRC
PDUs as commonly performed in real NR systems. In particular:
 ...
 * **SRB0** messages (over CCCH):
   ...
   - **RrcConnectionRequest**: in real NR systems, this is an RLC TM
   ...
   - **RrcConnectionSetup**: in the simulator this is implemented as in
     real NR systems, i.e., with an RLC TM SDU sent over resources
 ...
 * **SRB1** messages (over DCCH):
   ...
   - All the SRB1 messages modeled in the simulator (e.g.,
     **RrcConnectionCompleted**) are implemented as in real NR systems,
 ...
 * **SRB2** messages (over DCCH):
     ...
     - According to [TS36331]_, "*SRB1 is for RRC messages (which may
       ...
       requirement of the NR simulation model, hence we always use
...
The focus of the NR-NGC model is on the NAS Active state, which corresponds to EMM Registered, ECM connected, and RRC connected. Because of this, the following simplifications are made:

 - EMM and ECM are not modeled explicitly; instead, the NAS entity at the UE will interact directy with the AMF to perfom actions that are equivalent (with gross simplifications) to taking the UE to the states EMM Connected and ECM Connected; 
...
The N2-U interface is modeled in a realistic way by encapsulating
data packets over GTP/UDP/IP, as done in real NR-NGC systems. The
corresponding protocol stack is shown in Figure
:ref:`fig-nr-ngc-e2e-data-protocol-stack`. As shown in the figure,
...
NgcSmfUpfApplication. This application does the following operations:
 ...
 #. it classifies the packet using Traffic Flow Templates (TFTs) to
    ...
    one-to-one mapping to N2-U Bearers, so this operation returns the
    GTP-U Tunnel Endpoint Identifier  (TEID) to which the packet
    belongs;
 #. it adds the corresponding GTP-U protocol header to the packet;
 #. finally, it sends the packet over an UDP socket to the N2-U
...
and GTP headers is sent through one of the N2 links to the eNB, where
...
the following operations:
 ...
 #. it removes the GTP header and retrieves the TEID which is
    contained in it;
 #. leveraging on the one-to-one mapping between N2-U bearers and
 ...
 #. it forwards the packet to the NrEnbNetDevice of the eNB node via
    a raw packet socket

Note that, at this point, the outmost header of the packet is the
end-to-end IP header, since the IP/UDP/GTP headers of the N2 protocol
stack have already been stripped. Upon reception of
the packet from the NgcEnbApplication, the NrEnbNetDevice will
...
to the UE over the NR radio interface. Finally, the NrUeNetDevice of
...
and forwarded by the local TCP/IP stack to the NrUeNetDevice of the
UE. The NrUeNetDevice then performs the following operations:
 ...
 #. it identifies the corresponding PDCP protocol instance, which is
    the entry point of the NR Radio Protocol stack for this packet;
 #. it sends the packet to the eNB over the NR Radio Protocol stack.

The eNB receives the packet via its NrEnbNetDevice. Since there is a
single PDCP and RLC protocol instance for each Radio Bearer, the
NrEnbNetDevice is able to determine the BID of the packet. This BID
is then recorded onto an EpsBearerTag, which is added to the
packet. The NrEnbNetDevice then forwards the packet to the
...
following operations:
 ...
 #. it determines the corresponding EPS Bearer instance and GTP-U TEID by
    leveraging on the one-to-one mapping between N2-U bearers and Radio
    Bearers;
 #. it adds a GTP-U header on the packet, including the TEID
    determined previously;
 #. it sends the packet to the SMF/UPF node via the UDP socket
    connected to the N2-U point-to-point net device.

At this point, the packet contains the N2-U IP, UDP and GTP headers in
addition to the original end-to-end IP header. When the packet is
received by the corresponding N2-U point-to-point NetDevice of the
...
Note that the EPS Bearer QoS is not enforced on the N2-U
...
The N2-AP interface provides control plane interaction between the eNB
and the AMF. In the simulator, this interface is modeled in an ideal
fashion, with direct interaction between the eNB and the AMF objects,
without actually implementing the encoding of N2AP messages and
information elements specified in [TS36413]_ and without actually
transmitting any PDU on any link. 

The N2-AP primitives that are modeled are:
...
.. figure:: figures/nr-ngc-x2-handover-seq-diagram.*
   :scale: 80 %
   :align: center
...
attributes of the respective ``NrEnbRrc`` instances. When one of these timers
...
of NR module. Users should tune the simulation properly in order to avoid
...
.. figure:: figures/nr-ngc-x2-entity-saps.*
    :width: 700px
    :align: center
...
Figure :ref:`fig-nr-ngc-x2-interface` shows the protocol stacks of the X2-U interface and X2-C interface modeled in the simulator.

.. _fig-nr-ngc-x2-interface:

.. figure:: figures/nr-ngc-x2-interface.*          
    :align: center
...
procedure. Similarly to what done for the N2-U interface, data packets
...
can be used. We assume that the NrEnbRrc has been modified to include
...
mentioned. Then, you can add the following code in the NrEnbRrc class
...
``NrEnbRrc::DoRecvLoadInformation`` will be called when the target
...
status update primitive is used. We assume that the NrEnbRrc has been
...
The N11 interface provides control plane interaction between the SMF
and the AMF using the GTPv2-C protocol specified in [TS29274]_. In the
simulator, this interface is modeled in an ideal 
fashion, with direct interaction between the SMF and the AMF objects,
without actually implementing the encoding of the messages and without actually
transmitting any PDU on any link. 

The N11 primitives that are modeled are:
...
for the establishment of the N2-U bearers; the other two are used
during handover to switch the N2-U bearers from the source eNB to the
target eNB as a consequence of the reception by the AMF of a PATH
SWITCH REQUEST N2-AP message.
...
.. _fig-nr-downlik-power-control:
 
.. figure:: figures/nr-dl-power-control.*
   :align: center

   Sequence diagram of Downlink Power Control


Figure :ref:`fig-nr-downlik-power-control` shows the sequence diagram of 
...
When NrEnbPhy starts new subframe, DCI control messages are processed to get 
...
NrRrcSap::PhysicalConfigDedicated struct, which is used in 
 ...
 * :math:`P_{SRS\_OFFSET,c}(m)` is semi-statically configured by higher layers for :math:`m=0,1` for 
   ...
   This parameter is configurable by attribute system::
      ...
      Config::SetDefault ("ns3::NrUePowerControl::PsrsOffset", IntegerValue (7));
 ...
 * ``ns3::NrFrHardAlgorithm``
 * ``ns3::NrFrStrictAlgorithm``
 * ``ns3::NrFrSoftAlgorithm``
 * ``ns3::NrFfrSoftAlgorithm``
 * ``ns3::NrFfrEnhancedAlgorithm``
 * ``ns3::NrFfrDistributedAlgorithm``


New NrFfrAlgorithm class was created and it is a abstract class for 
Frequency Reuse algorithms implementation. Also, two new SAPs between 
FR-Scheduler and FR-RRC were added. 


.. _fig-nr-ffr-scheduling:
 
.. figure:: figures/nr-ffr-scheduling.*
   :align: center

   Sequence diagram of Scheduling with FR algorithm

Figure :ref:`fig-nr-ffr-scheduling` shows the sequence diagram of 
...
No Frequency Reuse
------------------
The NoOp FR algorithm (NrFrNoOpAlgorithm class) is implementation of 
...
is the simplest scheme and is the basic way of operating an NR network. This 
...
Figure :ref:`fig-nr-full-frequency-reuse-scheme` below presents frequency and 
power plan for Full Frequency Reuse scheme.

.. _fig-nr-full-frequency-reuse-scheme:
...
Figure :ref:`fig-nr-hard-frequency-reuse-scheme` below presents frequency and 
power plan for Hard Frequency Reuse scheme.

.. _fig-nr-hard-frequency-reuse-scheme:
...
Figure :ref:`fig-nr-strict-frequency-reuse-scheme` below presents frequency and 
power plan for Strict Frequency Reuse scheme with a cell-edge reuse factor of N = 3.

.. _fig-nr-strict-frequency-reuse-scheme:
 ...
 * In first version, the sub-band dedicated for the cell-edge UEs may also be used 
   ...
   Figure :ref:`fig-nr-soft-frequency-reuse-scheme-v1` below presents frequency and 
   power plan for this version of Soft Frequency Reuse scheme.

   .. _fig-nr-soft-frequency-reuse-scheme-v1:
 ...
 * In second version, cell-center UEs do not have access to cell-edge sub-band. 
   ...
   Figure :ref:`fig-nr-soft-frequency-reuse-scheme-v2` below presents frequency 
   and power plan for this version of Soft Frequency Reuse scheme. 

   .. _fig-nr-soft-frequency-reuse-scheme-v2:
...
Figure :ref:`fig-nr-soft-fractional-frequency-reuse-scheme` below presents 
frequency and power plan for Soft Fractional Frequency Reuse.

.. _fig-nr-soft-fractional-frequency-reuse-scheme:
...
Figure :ref:`fig-nr-enhanced-fractional-frequency-reuse-scheme` below presents 
frequency and power plan for Enhanced Fractional Frequency Reuse.

.. _fig-nr-enhanced-fractional-frequency-reuse-scheme:
...
Figure :ref:`fig-nr-distributed-fractional-frequency-reuse-scheme` below presents 
sequence diagram of Distributed Fractional Frequency Reuse Scheme.

.. _fig-nr-distributed-fractional-frequency-reuse-scheme:
 ...
 * ``NrHelper``, which takes care of the configuration of the NR radio
   access network, as well as of coordinating the setup and release of
   EPS bearers. The ``NrHelper`` class provides both the API
...
It is possible to create a simple NR-only simulations by
using ``NrHelper`` alone, or to create complete NR-NGC simulations by
using both ``NrHelper`` and ``NgcHelper``. When both helpers are used, they
interact in a master-slave fashion, with ``NrHelper`` being the Master
...
``NrHelper``. The exact interactions are displayed in the Figure :ref:`fig-helpers`.
...
.. figure:: figures/helpers.*
   ...
   Sequence diagram of the interaction between NrHelper and NgcHelper
