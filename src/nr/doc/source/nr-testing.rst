...
To test and validate the ns-3 NR module, several test suites are provided which are integrated with the ns-3 test framework.
To run them, you need to have configured the build of the simulator in this way::

    $ ./waf configure --enable-tests --enable-modules=nr --enable-examples
    $ ./test.py

The above will run not only the test suites belonging to the NR module, but also those belonging to all the other ns-3 modules on which the NR module depends. See the ns-3 manual for generic information on the testing framework.
...
The test suite ``nr-downlink-sinr`` 
...
The test suite ``nr-uplink-sinr`` checks that the SINR calculation in
uplink is performed correctly. This test suite is identical to
``nr-downlink-sinr`` described in the previous section, with the
...
The test suite ``nr-earfcn`` checks that the carrier frequency used
by the NrSpectrumValueHelper class (which implements the NR spectrum
...
returned by NrSpectrumValueHelper is the same as the known value for
...
-----------------------------------
The test suite ‘nr-test-deactivate-bearer’ creates test case with single EnodeB and Three UE’s.
...
The test suite ``nr-link-adaptation`` provides system tests recreating a
...
re-implementing in Octave (see `src/nr/test/reference/nr_amc.m`) the
model described in Section :ref:`sec-nr-amc` for the calculation of the
...
represented in Figure :ref:`fig-nr-mcs-index`.
...
which is calcuanrd by the simulator is also obtained using the
``NrChunkProcessor`` interface. The test
...
The test suite `nr-interference` provides system tests recreating an
...
is depicted in Figure :ref:`fig-nr-interference-test-scenario`. The
...
.. _fig-nr-interference-test-scenario:

.. figure:: figures/nr-interference-test-scenario.*
   :align: center
...
(available in
`src/nr/test/reference/nr_link_budget_interference.m`), which does
...
:ref:`sec-nr-amc-tests`. We note that the test vector
...
The test suite `nr-ue-measurements` provides system tests recreating an
inter-cell interference scenario identical of the one defined for
`nr-interference` test-suite. However, in this test the quantities to be
...
in `src/nr/test/reference/nr-ue-measurements.m`), which does the link budget
...
testing UE measurements: `nr-ue-measurements-piecewise-1`,
`nr-ue-measurements-piecewise-2`, and `nr-ue-measurements-handover`. These
...
measurement reports received by eNodeB. Each test case is an stand-alone NR
...
one is `nr-ue-measurements-piecewise-1`, henceforth Piecewise test #1, which
simulates 1 UE and 1 eNodeB. The other one is `nr-ue-measurements-piecewise-2`,
...
The `nr-ue-measurements-handover` test suite covers various types of
...
The test suite ``nr-rr-ff-mac-scheduler`` creates different test cases with
...
The test suite ``nr-pf-ff-mac-scheduler`` creates different test cases with
...
Test suites ``nr-fdmt-ff-mac-scheduler`` and ``nr-tdmt-ff-mac-scheduler`` 
...
Test suites ``nr-tta-ff-mac-scheduler``
create different test cases with a single eNB and several UEs, all having the same 
...
Test suites ``nr-tdbet-ff-mac-scheduler`` and ``nr-fdbet-ff-mac-scheduler`` create different
test cases with a single eNB and several UEs, all having the same Radio Bearer specification. 

In the first test case of ``nr-tdbet-ff-mac-scheduler`` and ``nr-fdbet-ff-mac-scheduler``, 
...
Test suites ``nr-fdtbfq-ff-mac-scheduler`` and ``nr-tdtbfq-ff-mac-scheduler`` create different
...
Test suites ``nr-pss-ff-mac-scheduler`` create different test cases with a single eNB and several UEs.
...
In the first class test case of ``nr-pss-ff-mac-scheduler``, the UEs are all placed at the same distance from
...
In the second class of test case of ``nr-pss-ff-mac-scheduler``, the UEs are all placed at the same distance from
...
the same test cases as the ``nr-pss-ff-mac-scheduler``. Additionally, in [Bbojovic2014]_ can be found performance 
...
BuildingPathlossModel with the nr module. The test exploits a set of
...
The test suite ``nr-phy-error-model`` generates different test cases for
...
Modulation and Coding (AMC) module, the ``NrAmc`` class, for making it less
...
The expected throughput is calcunrd by counting the number of transmission slots available during the simulation (e.g., the number of TTIs) and the size of the TB in the simulation, in detail:
...
The test suite `nr-antenna` checks that the AntennaModel integrated
with the NR model works correctly. This test suite recreates a
...
Two test suites ``nr-rlc-um-transmitter`` and
``nr-rlc-am-transmitter`` check that the UM RLC and the AM RLC
...
MAC and of the PDCP, implementing respectively the NrMacSapProvider
and NrRlcSapUser interfaces. Different test cases (i.e., input test
...
``nr-rlc-am-e2e``, which test the correct retransmission of RLC PDUs
...
where:
 ...
 - :math:`d^{ra}` is the delay for the MAC Random Access (RA)
   ...
   NrEnbMac::RaResponseWindowSize, which defaults to 3ms, plus 1ms
...
``NrRrcConnectionEstablishmentTestCase`` are evaluated - they are
...
The test suite `nr-cell-selection` is responsible for verifying the
...
:ref:`fig-nr-cell-selection-scenario` depicts the network layout and the
...
.. _fig-nr-cell-selection-scenario:

.. figure:: figures/nr-cell-selection-scenario.*
   :scale: 80 %
   :align: center
...
Two test suites (``ngc-n2u-uplink`` and ``ngc-n2u-downlink``) make
sure that the N2-U interface implementation works correctly in
isolation. This is achieved by creating a set of simulation scenarios
where the NGC model alone is used, without the NR model (i.e.,
without the NR radio protocol stack, which is replaced by simple CSMA
...
End-to-end NR-NGC data plane functionality
-------------------------------------------

The test suite ``nr-ngc-e2e-data`` ensures the correct end-to-end
functionality of the NR-NGC data plane. For each test case in this
suite, a complete NR-NGC simulation
...
The test suite ``nr-x2-handover`` checks the correct functionality of the X2 handover procedure. The scenario being tested is a topology with two eNBs connected by an X2 interface. Each test case is a particular instance of this scenario defined by the following parameters:
...
The test suite ``nr-x2-handover-measures`` checks the correct functionality of the handover
...
``nr-handover-delay`` verifies that this procedure consistently spends the
same amount of time.
...
testing a different subframe in NR. Thus the last test case has the handover
at +0.109s.
...
The test suite ``nr-handover-target`` verifies that the handover algorithm is
...
simulation time. Figure :ref:`fig-nr-handover-target-scenario` below
...
.. _fig-nr-handover-target-scenario:

.. figure:: figures/nr-handover-target-scenario.*
   :scale: 80 %
   :align: center

   ``nr-handover-target`` test scenario in a 2×2 grid
...
The test suite ``nr-downlink-power-control`` checks correctness of Downlink 
Power Control in three different ways: 

 * NrDownlinkPowerControlSpectrumValue test case   check if 
   ``NrSpectrumValueHelper::CreateTxPowerSpectralDensity`` is creating correct 
   ...
   ``NrSpectrumValueHelper::CreateTxPowerSpectralDensity`` is equal to expected TxPSD. 

 * NrDownlinkPowerControlTestCase test case check if TX power difference between 
   data and control channel is equal to configured PdschConfigDedicated::P_A value. 
   TX power of control channel is measured by ``NrTestSinrChunkProcessor`` added 
   ...
   ``NrTestSinrChunkProcessor`` is added to ``DataPowerChunkProcessor`` list in UE 
   DownlinkSpectrumPhy. Test vector contain a set of all avaiable P_A values. Test 
   pass if power diffrence equals P_A value. 

 * NrDownlinkPowerControlRrcConnectionReconfiguration test case check if 
...
The test suite ``nr-uplink-power-control`` verifies if Tx Power is computed correctly.
There are three different test cases: 

 * NrUplinkOpenLoopPowerControlTestCase test case checks Uplink Power Control functionality 
 ...
 * NrUplinkClosedLoopPowerControlAbsoluteModeTestCase test case checks Uplink Power Control 
   ...
   is not changing its position. NrFfrSimple algorithm is used on eNB side to set TPC values in 
 ...
 * NrUplinkClosedLoopPowerControlAccumulatedModeTestCase test case checks Closed Loop Uplink
   ...
   is not changing its position. As in above test case, NrFfrSimple algorithm is used on eNB 
...
The test suite ``nr-frequency-reuse`` contain two types of test cases. 
...
check which RBGs are used ``NrSimpleSpectrumPhy`` is attached to Downlink Channel. 
...
``NrSimpleSpectrumPhy`` is attached to Downlink Channel. It notifies when data 
...
The same approach is applied in Uplink direction and second ``NrSimpleSpectrumPhy``
...
The test suite ``nr-interference-fr`` is very similar to ``nr-interference``. 
Topology (Figure :ref:`fig-nr-interference-test-scenario`) is the same and test checks 
