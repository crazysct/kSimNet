...
#include <ns3/nr-rem-spectrum-phy.h>
...
NrRadioEnvironmentMapHelper::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::NrRadioEnvironmentMapHelper")
    ...
    .AddAttribute ("ChannelPath", "The path to the channel for which the Radio Environment Map is to be generated",
                   ...
                   MakeStringAccessor (&NrRadioEnvironmentMapHelper::m_channelPath),
                   MakeStringChecker ())
    ...
    .AddAttribute ("XMin", "The min x coordinate of the map.",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&NrRadioEnvironmentMapHelper::m_xMin),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("YMin", "The min y coordinate of the map.",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&NrRadioEnvironmentMapHelper::m_yMin),
                   MakeDoubleChecker<double> ())
   .AddAttribute ("XMax", "The max x coordinate of the map.",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&NrRadioEnvironmentMapHelper::m_xMax),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("YMax", "The max y coordinate of the map.",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&NrRadioEnvironmentMapHelper::m_yMax),
                   MakeDoubleChecker<double> ())
   .AddAttribute ("XRes", "The resolution (number of points) of the map along the x axis.",
                   UintegerValue (100),
                   MakeUintegerAccessor (&NrRadioEnvironmentMapHelper::m_xRes),
    ...
    .AddAttribute ("YRes", "The resolution (number of points) of the map along the y axis.",
                   UintegerValue (100),
                   MakeUintegerAccessor (&NrRadioEnvironmentMapHelper::m_yRes),
    ...
    .AddAttribute ("Z", "The value of the z coordinate for which the map is to be generated",
		   DoubleValue (0.0),
                   MakeDoubleAccessor (&NrRadioEnvironmentMapHelper::m_z),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("StopWhenDone", "If true, Simulator::Stop () will be called as soon as the REM has been generated",
		   BooleanValue (true),
                   MakeBooleanAccessor (&NrRadioEnvironmentMapHelper::m_stopWhenDone),
                   MakeBooleanChecker ())
    .AddAttribute ("NoisePower",
                   "the power of the measuring instrument noise, in Watts. Default to a kT of -174 dBm with a noise figure of 9 dB and a bandwidth of 25 NR Resource Blocks",
                   DoubleValue (1.4230e-13),
                   MakeDoubleAccessor (&NrRadioEnvironmentMapHelper::m_noisePower),
                   MakeDoubleChecker<double> ())
...
uint8_t 
NrRadioEnvironmentMapHelper::GetBandwidth () const
{
  return m_bandwidth;
}

void 
NrRadioEnvironmentMapHelper::SetBandwidth (uint8_t bw)
{
  switch (bw)
    { 
    case 6:
...
void 
NrRadioEnvironmentMapHelper::Install ()
{
  NS_LOG_FUNCTION (this);
  if (!m_rem.empty ())
    {
      NS_FATAL_ERROR ("only one REM supported per instance of NrRadioEnvironmentMapHelper");
    }
...
NrRadioEnvironmentMapHelper::DelayedInstall ()
{
  ...
  for (uint32_t i = 0; i < m_maxPointsPerIteration; ++i)
    {
      RemPoint p;
      p.phy = CreateObject<NrRemSpectrumPhy> ();
      ...
      p.phy->SetRxSpectrumModel (NrSpectrumValueHelper::GetSpectrumModel (m_earfcn, m_bandwidth));
  ...
  if (remIt != m_rem.end ())
    {
      ...
      NS_LOG_LOGIC ("deactivating NrRemSpectrumPhys that are unneeded in the last iteration");
