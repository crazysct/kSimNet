...
NS_LOG_COMPONENT_DEFINE ("NrRadioBearerStatsCalculator");

NS_OBJECT_ENSURE_REGISTERED ( NrRadioBearerStatsCalculator);

NrRadioBearerStatsCalculator::NrRadioBearerStatsCalculator ()
  : m_firstWrite (true),
...
NrRadioBearerStatsCalculator::NrRadioBearerStatsCalculator (std::string protocolType)
  : m_firstWrite (true),
    m_pendingOutput (false)
{
  NS_LOG_FUNCTION (this);
...
TypeId
NrRadioBearerStatsCalculator::GetTypeId (void)
{
  static TypeId tid =
    TypeId ("ns3::NrRadioBearerStatsCalculator")
    .SetParent<NrStatsCalculator> ().AddConstructor<NrRadioBearerStatsCalculator> ()
    .SetGroupName("Nr")
    .AddAttribute ("StartTime", "Start time of the on going epoch.", 
                   TimeValue (Seconds (0.)),
                   MakeTimeAccessor (&NrRadioBearerStatsCalculator::SetStartTime,
                                     &NrRadioBearerStatsCalculator::GetStartTime), 
                   MakeTimeChecker ())
    .AddAttribute ("EpochDuration", "Epoch duration.", 
                   TimeValue (Seconds (0.25)), 
                   MakeTimeAccessor (&NrRadioBearerStatsCalculator::GetEpoch,
                                     &NrRadioBearerStatsCalculator::SetEpoch), 
                   MakeTimeChecker ())
    .AddAttribute ("DlRlcOutputFilename",
                   "Name of the file where the downlink results will be saved.",
                   StringValue ("DlRlcStats.txt"),
                   MakeStringAccessor (&NrStatsCalculator::SetDlOutputFilename),
                   MakeStringChecker ())
    .AddAttribute ("UlRlcOutputFilename",
                   "Name of the file where the uplink results will be saved.",
                   StringValue ("UlRlcStats.txt"),
                   MakeStringAccessor (&NrStatsCalculator::SetUlOutputFilename),
                   MakeStringChecker ())
    .AddAttribute ("DlPdcpOutputFilename",
                   "Name of the file where the downlink results will be saved.",
                   StringValue ("DlPdcpStats.txt"),
                   MakeStringAccessor (&NrRadioBearerStatsCalculator::SetDlPdcpOutputFilename),
                   MakeStringChecker ())
    .AddAttribute ("UlPdcpOutputFilename",
                   "Name of the file where the uplink results will be saved.",
                   StringValue ("UlPdcpStats.txt"),
                   MakeStringAccessor (&NrRadioBearerStatsCalculator::SetUlPdcpOutputFilename),
                   MakeStringChecker ())
  ;
  return tid;
}
...
void
NrRadioBearerStatsCalculator::DoDispose ()
{
  NS_LOG_FUNCTION (this);
  if (m_pendingOutput)
    {
...
void 
NrRadioBearerStatsCalculator::SetStartTime (Time t)
{
  m_startTime = t;
  RescheduleEndEpoch ();
}
...
Time 
NrRadioBearerStatsCalculator::GetStartTime () const
{
  return m_startTime;
}

void 
NrRadioBearerStatsCalculator::SetEpoch (Time e)
{
  m_epochDuration = e;
  RescheduleEndEpoch ();
}
...
Time 
NrRadioBearerStatsCalculator::GetEpoch () const
{
  return m_epochDuration;  
}

void
NrRadioBearerStatsCalculator::UlTxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid, uint32_t packetSize)
{
  NS_LOG_FUNCTION (this << "UlTxPDU" << cellId << imsi << rnti << (uint32_t) lcid << packetSize);
  NrImsiLcidPair_t p (imsi, lcid);
  if (Simulator::Now () >= m_startTime)
    {
      m_ulCellId[p] = cellId;
      m_flowId[p] = NrFlowId_t (rnti, lcid);
      m_ulTxPackets[p]++;
...
void
NrRadioBearerStatsCalculator::DlTxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid, uint32_t packetSize)
{
...
void
NrRadioBearerStatsCalculator::UlRxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid, uint32_t packetSize,
                                     uint64_t delay)
{
...
void
NrRadioBearerStatsCalculator::DlRxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid, uint32_t packetSize, uint64_t delay)
{
...
void
NrRadioBearerStatsCalculator::ShowResults (void)
{
...
void
NrRadioBearerStatsCalculator::WriteUlResults (std::ofstream& outFile)
{
  NS_LOG_FUNCTION (this);

  // Get the unique IMSI / LCID list

  std::vector < NrImsiLcidPair_t > pairVector;
  ...
  for (std::vector<NrImsiLcidPair_t>::iterator it = pairVector.begin (); it != pairVector.end (); ++it)
    {
      NrImsiLcidPair_t p = *it;
...
void
NrRadioBearerStatsCalculator::WriteDlResults (std::ofstream& outFile)
{
  NS_LOG_FUNCTION (this);
  ...
  Time endTime = m_startTime + m_epochDuration;
  for (std::vector<NrImsiLcidPair_t>::iterator pair = pairVector.begin (); pair != pairVector.end (); ++pair)
    {
      NrImsiLcidPair_t p = *pair;
      outFile << m_startTime.GetNanoSeconds () / 1.0e9 << "\t";
      outFile << endTime.GetNanoSeconds () / 1.0e9 << "\t";
...
void
NrRadioBearerStatsCalculator::ResetResults (void)
{
  NS_LOG_FUNCTION (this);
...
void
NrRadioBearerStatsCalculator::RescheduleEndEpoch (void)
{
  NS_LOG_FUNCTION (this);
  m_endEpochEvent.Cancel ();
  NS_ASSERT (Simulator::Now ().GetMilliSeconds () == 0); // below event time assumes this
  m_endEpochEvent = Simulator::Schedule (m_startTime + m_epochDuration, &NrRadioBearerStatsCalculator::EndEpoch, this);
}

void
NrRadioBearerStatsCalculator::EndEpoch (void)
{
  NS_LOG_FUNCTION (this);
  ShowResults ();
  ResetResults ();
  m_startTime += m_epochDuration;
  m_endEpochEvent = Simulator::Schedule (m_epochDuration, &NrRadioBearerStatsCalculator::EndEpoch, this);
}

uint32_t
NrRadioBearerStatsCalculator::GetUlTxPackets (uint64_t imsi, uint8_t lcid)
{
...
uint32_t
NrRadioBearerStatsCalculator::GetUlRxPackets (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
...
uint64_t
NrRadioBearerStatsCalculator::GetUlTxData (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
  return m_ulTxData[p];
...
uint64_t
NrRadioBearerStatsCalculator::GetUlRxData (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
  return m_ulRxData[p];
...
double
NrRadioBearerStatsCalculator::GetUlDelay (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
  Uint64StatsMap::iterator it = m_ulDelay.find (p);
...
std::vector<double>
NrRadioBearerStatsCalculator::GetUlDelayStats (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
...
std::vector<double>
NrRadioBearerStatsCalculator::GetUlPduSizeStats (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
  std::vector<double> stats;
...
uint32_t
NrRadioBearerStatsCalculator::GetDlTxPackets (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
...
uint32_t
NrRadioBearerStatsCalculator::GetDlRxPackets (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
...
uint64_t
NrRadioBearerStatsCalculator::GetDlTxData (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
  return m_dlTxData[p];
...
uint64_t
NrRadioBearerStatsCalculator::GetDlRxData (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
  return m_dlRxData[p];
...
uint32_t
NrRadioBearerStatsCalculator::GetUlCellId (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
  return m_ulCellId[p];
...
uint32_t
NrRadioBearerStatsCalculator::GetDlCellId (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
  return m_dlCellId[p];
...
double
NrRadioBearerStatsCalculator::GetDlDelay (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
  Uint64StatsMap::iterator it = m_dlDelay.find (p);
...
std::vector<double>
NrRadioBearerStatsCalculator::GetDlDelayStats (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
  std::vector<double> stats;
...
std::vector<double>
NrRadioBearerStatsCalculator::GetDlPduSizeStats (uint64_t imsi, uint8_t lcid)
{
  NS_LOG_FUNCTION (this << imsi << (uint16_t) lcid);
  NrImsiLcidPair_t p (imsi, lcid);
  std::vector<double> stats;
...
std::string
NrRadioBearerStatsCalculator::GetUlOutputFilename (void)
{
...
std::string
NrRadioBearerStatsCalculator::GetDlOutputFilename (void)
{
  if (m_protocolType == "RLC")
    {
...
void
NrRadioBearerStatsCalculator::SetUlPdcpOutputFilename (std::string outputFilename)
{
  m_ulPdcpOutputFilename = outputFilename;
}

std::string
NrRadioBearerStatsCalculator::GetUlPdcpOutputFilename (void)
{
  return m_ulPdcpOutputFilename;
}
void
NrRadioBearerStatsCalculator::SetDlPdcpOutputFilename (std::string outputFilename)
{
  m_dlPdcpOutputFilename = outputFilename;
}

std::string
NrRadioBearerStatsCalculator::GetDlPdcpOutputFilename (void)
{
