...
NS_LOG_COMPONENT_DEFINE ("NrRetxStatsCalculator");

NS_OBJECT_ENSURE_REGISTERED ( NrRetxStatsCalculator);

NrRetxStatsCalculator::NrRetxStatsCalculator ()
{
...
TypeId
NrRetxStatsCalculator::GetTypeId (void)
{
  static TypeId tid =
    TypeId ("ns3::NrRetxStatsCalculator")
    .SetParent<Object> ().AddConstructor<NrRetxStatsCalculator> ()
    .SetGroupName("Nr")
    .AddAttribute ("DlRlcRetxFilename",
                   "Name of the file where the downlink retx results will be saved.",
                   StringValue ("DlRlcRetx.txt"),
                   MakeStringAccessor (&NrRetxStatsCalculator::m_retxDlFilename),
                   MakeStringChecker ())
    .AddAttribute ("UlRlcRetxFilename",
                   "Name of the file where the uplink retx results will be saved.",
                   StringValue ("UlRlcRetx.txt"),
                   MakeStringAccessor (&NrRetxStatsCalculator::m_retxUlFilename),
                   MakeStringChecker ())
    ;
  return tid;
}
...
void
NrRetxStatsCalculator::DoDispose ()
{
  NS_LOG_FUNCTION (this);
}

void
NrRetxStatsCalculator::RegisterRetxDl(uint64_t imsi, uint16_t cellId, 
...
void
NrRetxStatsCalculator::RegisterRetxUl(uint64_t imsi, uint16_t cellId, 
	uint16_t rnti, uint8_t lcid, uint32_t packetSize, uint32_t numRetx)
{
