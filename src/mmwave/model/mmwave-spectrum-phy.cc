...
MmWaveSpectrumPhy::StartRx (Ptr<SpectrumSignalParameters> params)
{
	...
	if (mmwaveDataRxParams!=0)
	{
		...
		{	// if the first cast is 0 (the device is MC) then this if will not be executed
			//std::cout<<"MmWaveSpectrumPhy::lteMmWave" <<std::endl;
			isAllocated = false;
		} 
		else if ((rxMcUe != 0) && (rxMcUe->GetMmWavePhy()->IsReceptionEnabled() == false) && !isAdditionalMmWave) //sjkang1015
		{	// this is executed if the device is MC and is transmitting
		//	std::cout<<this << " ---> MmWaveSpectrumPhy::firstMmWave" <<std::endl;
			isAllocated = false;
		}
		else if ((rxMcUe != 0) && (rxMcUe->GetMmWavePhy_2()->IsReceptionEnabled() == false) && isAdditionalMmWave)//sjkang1015
			{	// this is executed if the device is MC and is transmitting
		//	std::cout<<this << "---- >MmWaveSpectrumPhy::secondMmWave" <<std::endl;
				isAllocated = false;
			}
		if (isAllocated)
		{
	...
	else
	{
		...
		//NS_LOG_UNCOND("mmWave-spectrum-phy");
		if (DlCtrlRxParams!=0)
		{
		//	NS_LOG_UNCOND("mmWave-spectrum-phy "<<m_cellId<<"\t"<< DlCtrlRxParams->cellId);
			...
			if(itTb != m_expectedTbs.end ())
			{
				...
				else if (rxMcUe)
				{
					...
					if(mcUe != 0)
					{
						...
						if (isAdditionalMmWave)  mmWaveEnb = mcUe->GetMmWaveTargetEnb_2();
						else mmWaveEnb= mcUe->GetMmWaveTargetEnb();
...
MmWaveSpectrumPhy::StartTxDlControlFrames (std::list<Ptr<MmWaveControlMessage> > ctrlMsgList, Time duration)
{
  ...
  case RX_DATA:
	...
	case IDLE:
	{
		...
		txParams->psd = m_txPsd;
		txParams->cellId = m_cellId;
	//	std::cout<<this << " MmWaveSpectrumPhy::StartTxDlControlFrames: cellId-> " <<m_cellId <<std::endl;
		txParams->pss = true;
		txParams->ctrlMsgList = ctrlMsgList;
		txParams->txAntenna = m_antenna;
		m_channel->StartTx (txParams);
...
MmWaveSpectrumPhy::UpdateSinrPerceived (const SpectrumValue& sinr)
{
	NS_LOG_FUNCTION (this << sinr);
	 //double sinrAvg = Sum(sinr)/(sinr.GetSpectrumModel()->GetNumBands()); //sjkang
		//	 	std::cout << sinrAvg <<"\t" <<sinr.GetSpectrumModel()->GetNumBands()<< std::endl;
