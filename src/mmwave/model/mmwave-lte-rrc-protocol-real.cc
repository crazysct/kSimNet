...
MmWaveLteUeRrcProtocolReal::DoSendRrcConnectionSetupCompleted (LteRrcSap::RrcConnectionSetupCompleted msg)
{
  ...
  if (m_setupParameters.srb1SapProvider)
    {
      m_setupParameters.srb1SapProvider->TransmitPdcpSdu (transmitPdcpSduParameters); ////should track sjkang1117
    }
}
 ...
 NS_LOG_FUNCTION(this<<rrcDlCcchMessage.GetMessageType () );
  ...
  switch ( rrcDlCcchMessage.GetMessageType () )
    ...
    case 3:
      ...
      //std::cout<<"MmWaveLteRrcProtocolReal :: receiving RrcConnetion Setup to UeRrc"<<std::endl;
    ...
    //  std::cout <<"Ue received second Id " << rrcConnectToMmWaveHeader.GetMessage_secondMmWaveCellId()<<std::endl;
...
NS_LOG_FUNCTION(this);
  ...
  switch ( rrcDlDcchMessage.GetMessageType () )
    ...
    case 4:
      ...
      std::cout << "UE receives MmWaveLteUeRrcReconfiguration message " <<std::endl;
...
void  //for sending SIB2 information
      ...
      for (int j = 0; j < nDevs; ++j)
        {
          ...
          {
            ueRrc = ueDev->GetRrc ();              
            NS_LOG_FUNCTION ("considering UE IMSI from LTE but not MC " << ueDev->GetImsi () << " that has cellId " << ueRrc->GetCellId ());
          ...
          else
          {
            ...
            if (mcUeDev != 0)
            {
              ...
              if (ueRrc->GetCellId () == m_cellId)
              {       
                NS_LOG_FUNCTION ("sending SI to IMSI from LTE Enb" << mcUeDev->GetImsi ());
                ueRrc->GetLteUeRrcSapProvider ()->RecvSystemInformation (msg);
                Simulator::Schedule (RRC_REAL_MSG_DELAY, 
                                     &LteUeRrcSapProvider::RecvSystemInformation,
                                     ueRrc->GetLteUeRrcSapProvider (), 
              ...
              {
                NS_LOG_FUNCTION ("sending SI to IMSI from mmWave1 Enb" << mcUeDev->GetImsi ());
                Simulator::Schedule (RRC_REAL_MSG_DELAY, 
                                     &LteUeRrcSapProvider::RecvSystemInformation,
                                     mcUeDev->GetMmWaveRrc()->GetLteUeRrcSapProvider (), 
                                     msg); 
              }
              else if(mcUeDev->GetMmWaveRrc_2() != 0 && mcUeDev->GetMmWaveRrc_2()->GetCellId() == m_cellId) //sjkang
              {
                NS_LOG_FUNCTION ("sending SI to IMSI from mmWave2 Enb" << mcUeDev->GetImsi ());
                Simulator::Schedule (RRC_REAL_MSG_DELAY,
                                     &LteUeRrcSapProvider::RecvSystemInformation,
                                     mcUeDev->GetMmWaveRrc_2()->GetLteUeRrcSapProvider (),
                                     msg);
              }
            }
            else
...
MmWaveLteEnbRrcProtocolReal::DoSendRrcConnectionSetup (uint16_t rnti, LteRrcSap::RrcConnectionSetup msg)
{
 ...
 NS_LOG_FUNCTION(this);
  ...
  if (m_setupUeParametersMap.find (rnti) == m_setupUeParametersMap.end () )
    ...
    {
     // std::cout<<"MmWaveLteEnbRrcProtocolReal ::  sending RrcConnectionSetup messag to UE from Enb" << std::endl;
...
NS_LOG_FUNCTION(this);
  ...
  //std::cout << "Enb will send  " << secondMmWaveCellId<<std::endl;
   connectToMmWaveHeader.SetMessage(mmWaveId,secondMmWaveCellId); //sjkang
...
MmWaveLteEnbRrcProtocolReal::DoSendRrcConnectionReconfiguration (uint16_t rnti, LteRrcSap::RrcConnectionReconfiguration msg)
{
	  NS_LOG_FUNCTION (this);
	Ptr<Packet> packet = Create<Packet> ();
std::cout << "send SendRrcConnectionReconfiguration message  "<<std::endl;
  RrcConnectionReconfigurationHeader rrcConnectionReconfigurationHeader;
 ...
 NS_LOG_FUNCTION(this);
  ...
  switch ( rrcUlDcchMessage.GetMessageType () )
    ...
    case 5:
     ...
     // std::cout << "MmWaveLteRrcProtocolReal : receive secodary cell is connected" <<std::endl;
      m_enbRrcSapProvider->RecvRrcSecondaryCellInitialAccessSuccessful (params.rnti, rrcNotifyPair.second, rrcNotifyPair.first); // go to enb rrc
      break;
    }
}
