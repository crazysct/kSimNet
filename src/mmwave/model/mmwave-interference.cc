...
mmWaveInterference::ConditionallyEvaluateChunk ()
{
    ...
    {
		NS_LOG_INFO (this << " signal = " << *m_rxSignal << " allSignals = " << *m_allSignals << " noise = " << *m_noise);
		SpectrumValue interf =  (*m_allSignals) - (*m_rxSignal) + (*m_noise);
		//double m_allAvg = Sum(m_allSignals)/ (m_allSignals.GetSpetrumModel()->GetNumBands()):
		...
		//double sinrAvg = Sum(sinr)/(sinr.GetSpectrumModel()->GetNumBands()); //sjkang
		 ...
		 //double rxAvg = Sum(*m_rxSignal)/((*m_rxSignal).GetSpectrumModel()->GetNumBands()); //sjkang

		 //std::cout << this << "\t" <<rxAvg <<"\t" <<sinrAvg<< std::endl;
		// std::cout << *m_rxSignal << std::endl;
    }
}
