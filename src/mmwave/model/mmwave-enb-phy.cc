 ...
 NS_LOG_FUNCTION(this);
	...
	for(std::map<uint64_t, Ptr<NetDevice> >::iterator ue = m_ueAttachedImsiMap.begin(); ue != m_ueAttachedImsiMap.end(); ++ue)
	{
		...
		*(rxPsd) *= pathGainLinear;              
		...
		{
			//if (isAddtionalMmWavPhy) //sjkang
			//	beamforming->isAdditionalMmWavePhy=true;
			//else
			//	beamforming->isAdditionalMmWavePhy=false;
		...
		*totalReceivedPsd += *rxPsd;
		...
		{															// target not set yet
			...
			{
				//std::cout << this << "---->" << "0 target Enb is --->" <<isAddtionalMmWavPhy <<std::endl;
				//if(isAddtionalMmWavPhy)
			...
			//}//else {
				if((mcUeDev->GetMmWaveTargetEnb_2() != m_netDevice) && (mcUeDev->GetMmWaveTargetEnb_2() != 0) && isAddtionalMmWavPhy) //sjkang1117
			{
				//std::cout << this << "---->" << "1 target Enb is----> " <<isAddtionalMmWavPhy<< std::endl;
				//if(!isAddtionalMmWavPhy)
				txAntennaArray->ChangeBeamformingVector(mcUeDev->GetMmWaveTargetEnb_2());
				//std::cout << this << "---->" << "1 target Enb is----> " <<isAddtionalMmWavPhy<< std::endl;
	...
	for(std::map<uint64_t, Ptr<NetDevice> >::iterator ue = m_ueAttachedImsiMap.begin(); ue != m_ueAttachedImsiMap.end(); ++ue)
	{
		...
		*totalReceivedPsd += *rxPsd;
		...
		{															// target not set yet
			...
			{
				txAntennaArray->ChangeBeamformingVector(mcUeDev->GetMmWaveTargetEnb());
			}else if((mcUeDev->GetMmWaveTargetEnb_2() != m_netDevice) && (mcUeDev->GetMmWaveTargetEnb_2() != 0) && isAddtionalMmWavPhy ) //sjkang1117
			{
				txAntennaArray->ChangeBeamformingVector(mcUeDev->GetMmWaveTargetEnb_2());
			}
		}
...
NS_LOG_FUNCTION(this);
  ...
  std::list <Ptr<MmWaveControlMessage > > dciMsgList;
	...
	if(m_slotNum == 0) // DL control slot
	{
		...
		// TX control period
		//std::cout<<"MmWaveEnbPhy:: transimit packet to Enb sjkang vi DL control channel" <<std::endl;;
		...
		SendCtrlChannels(ctrlMsgs, slotPeriod-NanoSeconds(1.0)); // -1 ns ensures control ends before data period //sjkang will send control mnessage
	}
	...
	else if (currSlot.m_tddMode == SlotAllocInfo::DL) 	  // transmit DL slot
	{
		...
		//			}
		//std::cout<<"MmWaveEnbPhy:: transimit packet to Enb sjkang" <<std::endl;;
	...
	else if (currSlot.m_tddMode == SlotAllocInfo::UL)  // receive UL slot
	{
		...
		for (uint8_t i = 0; i < m_deviceMap.size (); i++)
		{
			...
			//if (mcUeDev->GetMmWavePhy()==0){
				if(isAddtionalMmWavPhy) //sjkang
			ueRnti = (ueDev != 0) ? (ueDev->GetPhy ()->GetRnti ()) : (mcUeDev->GetMmWavePhy_2()->GetRnti ());//sjkang
				else
					ueRnti = (ueDev != 0) ? (ueDev->GetPhy ()->GetRnti ()) : (mcUeDev->GetMmWavePhy()->GetRnti ());//sjkang
	...
	{ // update beamforming vectors (currently supports 1 user only)
		...
		for (uint8_t i = 0; i < m_deviceMap.size (); i++)
		{
			...
			else
			{
				...
				if(isAddtionalMmWavPhy)
					ueRnti = ueMcDev->GetMmWavePhy_2 () -> GetRnti (); //sjkang
				else
...
{
	/* Send Ctrl messages*/
	NS_LOG_FUNCTION (this<<"Send Ctrl"); //sjkang send control message
...
//  	int16_t sinrFp = LteFfConverter::double2fpS11dot3 (sinrdb);
  ...
  //this->m_deviceMap
  *stream_sinr<< Simulator::Now().GetSeconds() <<"\t" <<10*log10(sinrAvg) << std::endl; /// need to write by UE later . it is just uplink sinr for single UE
}

void
MmWaveEnbPhy::SetStream(std::ostream * str){
	stream_sinr = str;
}
void
