  ...
  .AddAttribute ("Imsi",
   ...
   .AddAttribute ("CsgId",
	...
	.AddAttribute ("MmWaveUeRrc",
                   ...
                   MakePointerChecker <LteUeRrc> ())
	.AddAttribute ("MmWaveUeRrc_2",
				                      "The RRC associated to the mmWave stack of this NetDevice",
				                      PointerValue (),
				                      MakePointerAccessor (&McUeNetDevice::m_mmWaveRrc_2),
				                      MakePointerChecker <LteUeRrc> ())   //sjkang
	...
	.AddAttribute ( "MmWaveUePhy_2",
										"The PHY associated to the mmWave stack of this NetDevice",
										PointerValue (),
										MakePointerAccessor (&McUeNetDevice::m_mmWavePhy_2),
										MakePointerChecker <MmWaveUePhy> ())
	...
	.AddAttribute ( "MmWaveUeMac_2",
										"The MAC associated to the mmWave stack of this NetDevice",
										PointerValue (),
										MakePointerAccessor (&McUeNetDevice::m_mmWaveMac_2),
										MakePointerChecker <MmWaveUeMac> ())
...
McUeNetDevice::DoDispose (void)
{
  ...
  if(m_mmWaveRrc_2 != 0){
	  m_mmWaveRrc_2->Dispose ();
  }
	m_mmWaveRrc = 0;
	m_mmWavePhy->Dispose ();
	m_mmWavePhy = 0;
	m_mmWaveRrc_2 = 0;
	m_mmWavePhy_2->Dispose ();
	m_mmWavePhy_2 = 0;
...
McUeNetDevice::Receive (Ptr<Packet> p)
{
 ...
 // NS_LOG_UNCOND("received data in McUeNetDevice");
...
McUeNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
	...
	// NS_LOG_UNCOND("send data in McUeNetDevice");
...
McUeNetDevice::UpdateConfig (void)
{
    ...
    if(m_mmWaveRrc_2 != 0){
    	m_mmWaveRrc_2->SetImsi (m_imsi); //sjkang1117
    }
...
Ptr<MmWaveUePhy>
McUeNetDevice::GetMmWavePhy_2 (void) const //sjkang
{
	return m_mmWavePhy_2;
}

Ptr<MmWaveUeMac>
McUeNetDevice::GetMmWaveMac_2 (void) const //sjkang
{
	return m_mmWaveMac_2;
}

Ptr<LteUeRrc>
McUeNetDevice::GetMmWaveRrc_2 (void) const  //sjkang
{
  NS_LOG_FUNCTION (this);
  return m_mmWaveRrc_2;
}
...
{
	std::cout << "UE "<<m_imsi<<" set first target mmWave eNB " <<std::endl;
	m_mmWaveTargetEnb = enb;
}
void
McUeNetDevice::SetMmWaveTargetEnb_2 (Ptr<MmWaveEnbNetDevice> enb) //sjkang
{
	std::cout << "UE "<<m_imsi<<" set second target mmWave eNB " <<std::endl;
	m_mmWaveTargetEnb_2 = enb;
}
...
Ptr<MmWaveEnbNetDevice>
McUeNetDevice::GetMmWaveTargetEnb_2 (void) //sjkang
{
	return m_mmWaveTargetEnb_2;
}
